...
class Test_CBitcoinExtPubKey(unittest.TestCase):
    def test(self) -> None:
        ...
        T(
            "xpub661MyMwAqRbcFMfe2ZGFSPef9xMXWrZUDta7RXKPbtxuNyepg8ewAWVV5qME4omB67Ek4eDrpyFtMcUcznxCf8sV8DCnsZeWj6Z2N3RXqPo",
    ...
    def test_derive(self) -> None:
        ...
        T(
            "xpub661MyMwAqRbcG2veuy7DxC7yfKodTbY46UKYgnrERu9WADL5pBjRzthJtxMpWYeofw5rHWtemkgJAcSCup19Vorze6H3etYGbiEU4MumRV7",
    ...
    def test_invalid_xpubkey(self) -> None:
        ...
        valid_xpubkey = CBitcoinExtPubKey(
            "xpub661MyMwAqRbcG2veuy7DxC7yfKodTbY46UKYgnrERu9WADL5pBjRzthJtxMpWYeofw5rHWtemkgJAcSCup19Vorze6H3etYGbiEU4MumRV7"
        )
        with self.assertRaises(ValueError):
...
class Test_CExtKey(unittest.TestCase):
    def test_extkey(self) -> None:
        ...
        self.assertEqual(
            bytes(xprv.derive(BIP32_HARDENED_KEY_OFFSET)),
...
class Test_BIP32Path(unittest.TestCase):
    ...
    def test_path_as_list(self) -> None:
        ...
        self.assertEqual(
            list(BIP32Path("m/0'/2147483647'/1/10")), [BIP32_HARDENED_KEY_OFFSET, 0xFFFFFFFF, 1, 10]
        )
        ...
        self.assertEqual(
            list(
                BIP32Path(
                    "m/"
                    + "/".join("%u'" % n for n in range(128))
                    + "/"
                    + "/".join("%u" % (BIP32_HARDENED_KEY_OFFSET - n - 1) for n in range(127))
                )
            ),
    ...
    def test_tempate_as_list(self) -> None:
        ...
        self.assertEqual(
            list(BIP32PathTemplate("{0-10,12,59}/*")),
            [((0, 10), (12, 12), (59, 59)), ((0, BIP32_HARDENED_KEY_OFFSET - 1),)],
        )
        ...
        self.assertEqual(
            ...
            [((n + BIP32_HARDENED_KEY_OFFSET, n + BIP32_HARDENED_KEY_OFFSET),) for n in range(128)]
            + [
    ...
    def test_path_from_list(self) -> None:
        ...
        self.assertEqual(str(BIP32Path(BIP32Path([0], is_partial=False))), "m/0")
        ...
        self.assertEqual(
            str(BIP32Path([0xFFFFFFFF, 0x80000001, 1, 0x80000002])), "2147483647'/1'/1/2'"
        )

        self.assertEqual(
            str(BIP32Path([0xFFFFFFFF, 0x80000001, 1, 2], hardened_marker="h", is_partial=False)),
        ...
        self.assertEqual(
            str(
                BIP32Path(
                    [n + BIP32_HARDENED_KEY_OFFSET for n in range(128)] + [n for n in range(127)],
                    is_partial=False,
                )
            ),
            "m/"
    ...
    def test_path_template_from_list(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "unsupported hardened_marker"):
            # only apostrophe and "h" markers are allowed
            BIP32PathTemplate(
                [[(0xFFFFFFFF, 0xFFFFFFFF)], [(0, 0)], [(0x80000000, 0x80000000)]],
                hardened_marker="b",
            )
        ...
        self.assertEqual(str(BIP32PathTemplate([((0, 0),)], is_partial=False)), "m/0")
        self.assertEqual(str(BIP32PathTemplate([((0, 1),)], is_partial=True)), "{0-1}")
        self.assertEqual(str(BIP32PathTemplate([((0, 0),)], is_partial=True)), "0")

        self.assertEqual(
            str(BIP32PathTemplate([((0, BIP32_HARDENED_KEY_OFFSET - 1),)], is_partial=False)), "m/*"
        )

        self.assertEqual(
            str(BIP32PathTemplate([((BIP32_HARDENED_KEY_OFFSET, 0xFFFFFFFF),)], is_partial=False)),
            "m/*'",
        )
        self.assertEqual(
            str(
    ...
    def test_from_BIP32PathTemplate(self) -> None:
        ...
        self.assertEqual(str(BIP32PathTemplate(p)), "m/{4-44}h/{5-555555}h/1/4/*h")
        self.assertEqual(
            str(BIP32PathTemplate(p, hardened_marker="'")), "m/{4-44}'/{5-555555}'/1/4/*'"
        )
        p = BIP32PathTemplate("m/4'/5'/*/4")
        self.assertEqual(str(BIP32PathTemplate(p)), "m/4'/5'/*/4")
        self.assertEqual(str(BIP32PathTemplate(p, hardened_marker="h")), "m/4h/5h/*/4")
        p = BIP32PathTemplate("4'/5'/1/{3,4,5-10}")
        self.assertEqual(str(BIP32PathTemplate(p)), "4'/5'/1/{3-10}")
        self.assertEqual(str(BIP32PathTemplate(p, hardened_marker="h")), "4h/5h/1/{3-10}")
    ...
    def test_random_access(self) -> None:
        ...
        pt = BIP32PathTemplate([[(0xFFFFFFFF - n, 0xFFFFFFFF - n)] for n in range(255)])
    ...
    def test_BIP32PathTemplate_match_path(self) -> None:
        ...
        self.assertTrue(
            BIP32PathTemplate("m/4'/5'/1/{3,4,5-50}/*").match_path(BIP32Path("m/4'/5'/1/3/1"))
        )
        ...
        self.assertFalse(
            BIP32PathTemplate("4h/5h/1h/{3,4,5-50}h/*h").match_path(BIP32Path("4'/5'/1'/3/323452'"))
        )
        ...
        for status, data in test_dict.items():
            if status == "normal_finish":
                for elt in data:
                    ...
                    tmpl_str, tmpl = elt
                    tmpl_tuple = tuple(
                        tuple(tuple(range) for range in section) for section in json.loads(tmpl)
                    )
