...
class Test_KeyStore(unittest.TestCase):
    def test(self) -> None:
        ...
        kstore = KeyStore(
            external_privkey_lookup=(
                lambda key_id, dinfo: priv1 if key_id == priv1.pub.key_id else None
            ),
            external_pubkey_lookup=(lambda key_id, dinfo: pub1 if key_id == pub1.key_id else None),
        )
        ...
        kstore = KeyStore(xpriv1, priv1, xpub1, pub1, require_path_templates=False)
        self.assertEqual(kstore.get_privkey(priv1.pub.key_id), priv1)
        self.assertEqual(kstore.get_pubkey(priv1.pub.key_id), priv1.pub)
        self.assertEqual(kstore.get_pubkey(pub1.key_id), pub1)
        ...
        self.assertEqual(
            kstore.get_privkey(
                xpriv1.pub.key_id, KeyDerivationInfo(xpriv1.fingerprint, BIP32Path("m"))
            ),
            xpriv1.priv,
        )
        ...
        self.assertEqual(
            kstore.get_pubkey(
                xpub1.pub.key_id, KeyDerivationInfo(xpub1.parent_fp, BIP32Path("m/0"))
            ),
            xpub1.pub,
        )
        ...
        self.assertEqual(
            kstore.get_pubkey(xpub1.pub.key_id, KeyDerivationInfo(xpub1.parent_fp, BIP32Path("m"))),
            None,
        )

        # check longer derivations
        self.assertEqual(kstore.get_privkey(xpriv1.derive_path("0'/1'/2'").pub.key_id), None)
        self.assertEqual(
            kstore.get_privkey(
                xpriv1.derive_path("0'/1'/2'").pub.key_id,
                KeyDerivationInfo(xpriv1.fingerprint, BIP32Path("m/0'/1'/2'")),
            ),
        ...
        self.assertEqual(
            kstore.get_pubkey(
                derived_pub.key_id, KeyDerivationInfo(xpriv2.parent_fp, BIP32Path("m/0/3'/4'/5'"))
            ),
            derived_pub,
        )
        ...
        self.assertEqual(
            kstore.get_privkey(
                ...
                KeyDerivationInfo(xpriv2.parent_fp, BIP32Path("m/0/3'/4'/5'")),
            ),
            derived_priv,
        )
        ...
        self.assertEqual(kstore.get_pubkey(xpub1.derive_path("0/1/2").pub.key_id), None)
    ...
    def test_path_template_enforcement(self) -> None:
        ...
        xpub3.assign_derivation_info(KeyDerivationInfo(x("abcdef10"), BIP32Path("m/0/0")))

        # No error when require_path_templates is not set
        KeyStore(xpriv1, xpriv2, priv1, xpub1, pub1, require_path_templates=False)
        ...
        self.assertEqual(
            ks.get_privkey(priv1.pub.key_id, KeyDerivationInfo(xpriv1.parent_fp, BIP32Path("m"))),
            priv1,
        )
        self.assertEqual(ks.get_pubkey(pub1.key_id), pub1)
        ...
        self.assertEqual(
            ks.get_pubkey(xpub1.pub.key_id, KeyDerivationInfo(xpub1.parent_fp, BIP32Path("m/0"))),
            xpub1.pub,
        )
        ...
        with self.assertRaises(BIP32PathTemplateViolation):
            ks.get_pubkey(
                xpub1.derive(1).pub.key_id, KeyDerivationInfo(xpub1.parent_fp, BIP32Path("m/0/1"))
            )

        # success when template allows
        self.assertEqual(
            ks.get_pubkey(
                xpub3.derive(1).pub.key_id, KeyDerivationInfo(x("abcdef10"), BIP32Path("m/0/0/1"))
            ),
            xpub3.derive(1).pub,
        )
        ...
        with self.assertRaises(BIP32PathTemplateViolation):
            ks.get_pubkey(
                xpub3.derive(2).pub.key_id, KeyDerivationInfo(x("abcdef10"), BIP32Path("m/0/0/2"))
            )

        long_path = BIP32Path("m/43435/646/5677/5892/58885/2774/9943/75532/8888")
        ...
        with self.assertRaises(BIP32PathTemplateViolation):
            ks.get_privkey(
                ...
                KeyDerivationInfo(xpriv2.fingerprint, BIP32Path("m/44h/0h/0h/3/25")),
            )

        with self.assertRaises(BIP32PathTemplateViolation):
            ks.get_privkey(
                xpriv2.derive_path("44'/0'/0'/0/1'").pub.key_id,
                KeyDerivationInfo(xpriv2.fingerprint, BIP32Path("m/44h/0h/0h/0/1h")),
            )

        self.assertEqual(
            ks.get_privkey(
                xpriv2.derive_path("44'/0'/0'/1/25").pub.key_id,
                KeyDerivationInfo(xpriv2.fingerprint, BIP32Path("m/44h/0h/0h/1/25")),
            ),
        ...
        with self.assertRaisesRegex(ValueError, "must specify full path"):
            ks = KeyStore(xpriv2, xpub49, default_path_template="{44,49,84}h/0h/0h/{0-1}/{0-50000}")

        ks = KeyStore(xpriv2, xpub49, default_path_template="m/{44,49,84}h/0h/0h/{0-1}/{0-50000}")
        ...
        with self.assertRaises(BIP32PathTemplateViolation):
            ks.get_privkey(
                ...
                KeyDerivationInfo(xpriv2.fingerprint, BIP32Path("m/44h/0h/0h/1/50001")),
            )
        ...
        with self.assertRaises(BIP32PathTemplateViolation):
            ks.get_pubkey(
                ...
                KeyDerivationInfo(xpriv2.fingerprint, BIP32Path("m/49h/0h/0h/0/50001")),
            )

        with self.assertRaises(BIP32PathTemplateViolation):
            ks.get_pubkey(
                xpub49.derive_path("50000/3").pub.key_id,
                KeyDerivationInfo(xpriv2.fingerprint, BIP32Path("m/49h/0h/0h/0/50000/3")),
            )

        self.assertEqual(
            ks.get_pubkey(
                xpub49.derive_path("50000").pub.key_id,
                KeyDerivationInfo(xpriv2.fingerprint, BIP32Path("m/49h/0h/0h/0/50000")),
            ),
