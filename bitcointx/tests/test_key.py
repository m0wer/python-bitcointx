...
class Test_CPubKey(unittest.TestCase):
    def test(self) -> None:
        def T(hex_pubkey: str, is_nonempty: bool, is_fullyvalid: bool, is_compressed: bool) -> None:
        ...
        T("0378d430274f8c5ec1321338151e9f27f4c676a008bdf8638d07c0b6be9ab35c71", True, True, True)
        T("0478d430274f8c5ec1321338151e9f27f4c676a008bdf8638d07c0b6be9ab35c71", True, False, True)
...
class Test_CKey(unittest.TestCase):
    ...
    def test_add_sub(self) -> None:
        ...
        if secp256k1.cap.has_pubkey_negate:
            ...
            self.assertEqual(CPubKey.add(k2.pub, k2.pub), CPubKey.sub(pub_sum, pub_diff))
            self.assertEqual(k1, CKey.combine(k1, k2, k_sum, k2.negated(), k_sum.negated()))
            self.assertEqual(
                k1.pub,
                CPubKey.combine(k1.pub, k2.pub, k_sum.pub, k2.pub.negated(), k_sum.pub.negated()),
            )
            self.assertEqual(
                CKey.combine(k_sum, k2, k1, k_diff), CKey.combine(k1, k2, k_sum, k_diff)
            )
            self.assertEqual(
            ...
            with self.assertRaises(ValueError):
                CPubKey.combine(k1.pub, k2.pub, k1.pub.negated(), k2.pub.negated())
        else:
            warnings.warn(
    ...
    def test_signature_grind(self) -> None:
        ...
        high_r_found = False
        for i in range(1, 21):
            sig = k.sign(msg_hash, _ecdsa_sig_grind_low_r=False, _ecdsa_sig_extra_entropy=i)
            if sig[3] == 0x21:
        ...
        with open(os.path.dirname(__file__) + "/data/schnorr-sig-test-vectors.csv", "r") as fd:
            ...
            for row in reader:
                ...
                if seckey_hex != "":
                    ...
                    sig_actual = seckey.sign_schnorr_no_tweak(msg, aux=aux_rand)
