...
class Test_EvalScript(unittest.TestCase):
    ...
    def generate_taproot_test_scripts(self) -> TestDataIterator:
        ...
        nValue = 100
        (txCredit, txSpend) = self.create_test_txs(CScript(), spk, spk, CScriptWitness(), nValue)
        ...
        t = TaprootScriptTree([CScript([rnd_twpub, OP_CHECKSIG], name="simplespend")])
        ...
        yield (
            ...
            spk,
            spk,
            CScriptWitness([sig, s, cb]),
            nValue,
            txCredit.vout,
            BITCOINCONSENSUS_ACCEPTED_FLAGS,
            "OK",
            "",
            "simple taproot script spend",
        )

        def gen_leaves(
            num_leaves: int, prefix: str = ""
        ) -> Tuple[List[TaprootScriptTreeLeaf_Type], Dict[str, bytes]]:
        ...
        def yield_leaves(
            leaves: Sequence[Union[CScript, TaprootScriptTree]], tweaks: Dict[str, bytes]
        ) -> TestDataIterator:
            ...
            for sname, tweak in tweaks.items():
                ...
                yield (
                    ...
                    "OK",
                    "",
                    f"taproot script spend leaf {sname}",
                )
    ...
    def create_test_txs(
        ...
        dst_scriptPubKey: CScript,
        witness: CScriptWitness,
        nValue: int,
    ) -> Tuple[CTransaction, CTransaction]:
        txCredit = CTransaction(
        ...
        txSpend = CTransaction(
            [CTxIn(COutPoint(txCredit.GetTxid(), 0), scriptSig, nSequence=0xFFFFFFFF)],
    ...
    def test_script(self) -> None:
        for t in load_test_vectors("script_tests.json"):
            ...
            try:
                VerifyScript(
                    scriptSig, scriptPubKey, txSpend, 0, flags, amount=nValue, witness=witness
                )
    ...
    def _do_test_bicoinconsensus(
        self, handle: Optional[ctypes.CDLL], test_data_iterator: TestDataIterator
    ) -> None:
    ...
    def test_script_bitcoinconsensus(self) -> None:
        ...
        test_data_iterator = load_test_vectors("script_tests.json", skip_fixme=False)
        # test with supplied handle
        self._do_test_bicoinconsensus(self._bitcoinconsensus_handle, test_data_iterator)
    ...
    def test_p2sh_redeemscript(self) -> None:
        def T(required: int, total: int, alt_total: Optional[int] = None) -> None:
            amount = 10000
            keys = [CKey.from_secret_bytes(os.urandom(32)) for _ in range(total)]
            ...
            (_, tx) = self.create_test_txs(
                CScript(), scriptPubKey, CScript(), CScriptWitness([]), amount
            )

            tx = tx.to_mutable()

            sighash = redeem_script.sighash(
                tx, 0, SIGHASH_ALL, amount=amount, sigversion=SIGVERSION_BASE
            )

            sigs = [k.sign(sighash) + bytes([SIGHASH_ALL]) for k in keys[:required]]

            tx.vin[0].scriptSig = CScript(standard_multisig_witness_stack(sigs, redeem_script))

            VerifyScript(tx.vin[0].scriptSig, scriptPubKey, tx, 0, (SCRIPT_VERIFY_P2SH,))
            ...
            sighash = redeem_script.sighash(
                tx, 0, SIGHASH_ALL, amount=amount, sigversion=SIGVERSION_WITNESS_V0
            )
