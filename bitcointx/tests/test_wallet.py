            ...
            def recursive_check(aclass: type) -> None:
                ...
                if extra_addr_testfunc(aclass, pub):
                    ...
                    if getattr(aclass, "from_xonly_pubkey", None):
                        ...
                        a_from_spk = aclass.from_scriptPubKey(CScript(b"\x51\x20" + pub[1:]))
...
class Test_CCoinAddress(unittest.TestCase):
    ...
    def test_get_output_size(self) -> None:
        ...
        a2 = P2SHCoinAddress.from_redeemScript(CScript(b"\xa9" + Hash160(pub) + b"\x87"))
        self.assertEqual(P2SHCoinAddress.get_output_size(), 32)
        self.assertEqual(a2.get_output_size(), 32)
        a3 = P2WSHCoinAddress.from_redeemScript(CScript(b"\xa9" + Hash160(pub) + b"\x87"))
...
class Test_CBitcoinAddress(unittest.TestCase):
    def test_create_from_string(self) -> None:
        ...
        def T(
            str_addr: str, expected_bytes: bytes, expected_version: int, expected_class: type
        ) -> None:
        ...
        T(
            ...
            x("751e76e8199196d454941c45d1b3a323f1433bd6"),
            0,
            P2WPKHBitcoinAddress,
        )
    ...
    def test_from_scriptPubKey(self) -> None:
        def T(hex_scriptpubkey: str, expected_str_address: str, expected_class: type) -> None:
    ...
    def test_from_invalid_scriptPubKey(self) -> None:
        ...
        scriptPubKey = CScript(
            x("79be667ef9dcbbac55a06295ce870b07029bfcdb2dce28d959f2815b16f81798ac")
        )
        with self.assertRaises(CBitcoinAddressError):
            CBitcoinAddress.from_scriptPubKey(scriptPubKey)
        ...
        # One byte changed
        scriptPubKey = CScript(
            x("79be667ef9dcbbac55a06295ce870b07029bfcdb1dce28d959f2815b16f81798")
        )
        with self.assertRaises(CBitcoinAddressError):
            CBitcoinAddress.from_scriptPubKey(scriptPubKey)
    ...
    def test_to_redeemScript_ok(self) -> None:
        ...
        T("1111111111111111111114oLvT2", "76a914000000000000000000000000000000000000000088ac")
    ...
    def test_to_scriptPubKey(self) -> None:
        ...
        T("31h1vYVSYuKP6AhS86fbRdMw9XHieotbST", "a914000000000000000000000000000000000000000087")
    ...
    def test_from_redeemScript(self) -> None:
        ...
        T(
            CScript(x("76a914751e76e8199196d454941c45d1b3a323f1433bd688ac")),
    ...
    def test_from_valid_pubkey(self) -> None:
        ...
        T(
            x("78d430274f8c5ec1321338151e9f27f4c676a008bdf8638d07c0b6be9ab35c71"),
            "bc1p0r2rqf60330vzvsn8q23a8e87nr8dgqghhux8rg8czmtax4nt3csc0rfcn",
            P2TRBitcoinAddress,
        )
    ...
    def test_from_invalid_pubkeys(self) -> None:
        ...
        def T(
            invalid_pubkey: bytes,
            expected_str_addr: str,
            cls: Union[
                Type[P2PKHBitcoinAddress], Type[P2WPKHBitcoinAddress], Type[P2TRBitcoinAddress]
            ],
        ) -> None:
            addr: Union[P2PKHBitcoinAddress, P2WPKHBitcoinAddress, P2TRBitcoinAddress]
            if issubclass(cls, P2TRCoinAddress):
        ...
        T(inv_pub_bytes, "bc1q6gzj82cpgy0pe9jgh0utalfp2kyvvm72m0zlrt", P2WPKHBitcoinAddress)
        T(
            b"\x00" * 32,
        ...
        T(
            CPubKey(inv_pub_bytes),
            "bc1p0r2rqf60330vzvsn8q23a8e87nr8dgqghhux8rg8czmtax4nt3eqznytxx",
            P2TRBitcoinAddress,
        )
        ...
        for inv_pub in (x(""), inv_pub_bytes, CPubKey(inv_pub_bytes)):
            for cls in (P2PKHBitcoinAddress, P2WPKHBitcoinAddress, P2TRBitcoinAddress):
                with self.assertRaises(CBitcoinAddressError):
                    if issubclass(cls, P2TRCoinAddress):
        ...
        for inv_pub in (x(""), inv_pub_bytes[1:], XOnlyPubKey(inv_pub_bytes[1:])):
            with self.assertRaises(CBitcoinAddressError):
...
class Test_P2PKHBitcoinAddress(unittest.TestCase):
    def test_from_non_canonical_scriptPubKey(self) -> None:
        ...
        T(
            "76a94e14000000000000000000000000000000000000000000000088ac",
            "1111111111111111111114oLvT2",
        )
...
class Test_CBitcoinKey(unittest.TestCase):
    def test(self) -> None:
        def T(
            base58_privkey: str, expected_hex_pubkey: str, expected_is_compressed_value: bool
        ) -> None:
    ...
    def test_from_to_compressed(self) -> None:
        ...
        T(
            "0de5306487851213f0aae1454f4e4449949a755802b60f6eb47906149395d080",
        ...
        T(
            "c9ff05edfbfb4710267ccf212fbb0414284b09fce621f8ab61a5b1cf0f3a5bf2",
    ...
    def test_chain_params_context_manager(self) -> None:
        with ChainParams(BitcoinRegtestParams) as p1:
            ...
            with ChainParams(BitcoinSignetParams) as p2:
                ...
                assert isinstance(get_current_chain_params(), BitcoinSignetParams)
...
class Test_BIP341_standard_vectors(unittest.TestCase):
    ...
    def test_bip341_scriptPubKey(self) -> None:
        for tcase in self.spk_cases:
            ...
            if stree_data:
                    ...
                    for ld in leaves_data:
                        ...
                        if isinstance(ld, dict):
                            ...
                            if ld["leafVersion"] != CoreCoinParams.TAPROOT_LEAF_TAPSCRIPT:
                                leaf = TaprootScriptTree([leaf], leaf_version=ld["leafVersion"])
                        else:
                ...
                stree = TaprootScriptTree(process_leaves(stree_data), internal_pubkey=int_pub)
    ...
    def test_bip341_keyPathSpending(self) -> None:
        for tcase in self.keypath_spending_cases:
            tx = CMutableTransaction.deserialize(x(tcase["given"]["rawUnsignedTx"]))
            spent_outputs = [
                CTxOut(u["amountSats"], CScript(x(u["scriptPubKey"])))
            ...
            for inp_tcase in tcase["inputSpending"]:
                ...
                self.assertEqual(k.xonly_pub.hex(), intermediary["internalPubkey"])
                ...
                sh = SignatureHashSchnorr(tx, in_idx, spent_outputs, hashtype=ht)
                ...
                self.assertEqual([elt.hex() for elt in wstack], expected["witness"])

                tx.wit.vtxinwit[in_idx] = CMutableTxInWitness(CScriptWitness(wstack))

            signed_tx = CTransaction.deserialize(x(tcase["auxiliary"]["fullySignedTx"]))
            ...
            self.assertEqual(tx.serialize().hex(), tcase["auxiliary"]["fullySignedTx"])
