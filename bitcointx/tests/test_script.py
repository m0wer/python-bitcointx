...
class Test_CScript(unittest.TestCase):
    def test_sighash(self) -> None:
        ...
        tx = CTransaction(
            [CTxIn(COutPoint(b"\x00" * 32, 0), CScript([]), nSequence=0xFFFFFFFF)],
        ...
        spk_legacy.sighash(
            tx, 0, SIGHASH_ALL, amount=spent_amount, sigversion=SIGVERSION_WITNESS_V0
        )
        spk_segwit.sighash(
            tx, 0, SIGHASH_ALL, amount=spent_amount, sigversion=SIGVERSION_WITNESS_V0
        )
        ...
        with self.assertRaises(ValueError):
            # unknown sigversion
            spk_segwit.sighash(
                tx, 0, SIGHASH_ALL, amount=spent_amount, sigversion=SIGVERSION_WITNESS_V0 + 1
            )  # type: ignore
        ...
        with self.assertRaises(ValueError):
            ...
            spk_segwit.sighash(tx, 0, SIGHASH_ALL, amount=spent_amount, sigversion=SIGVERSION_BASE)

        with self.assertRaises(ValueError):
            # inIdx > len(tx.vin) - non-raw sighash function should raise
            # ValueError (raw_sighash can return HASH_ONE)
            spk_legacy.sighash(tx, 10, SIGHASH_ALL, amount=spent_amount, sigversion=SIGVERSION_BASE)
    ...
    def test_parse_standard_multisig_redeem_script(self) -> None:
        ...
        pubkeys = [CKey.from_secret_bytes(os.urandom(32)).pub for _ in range(15)]

        T2(
            CScript([1, pubkeys[0], pubkeys[1], 2, OP_CHECKMULTISIG]),
            StandardMultisigScriptInfo(total=2, required=1, pubkeys=pubkeys[:2]),
        )
        ...
        T2(
            ...
            StandardMultisigScriptInfo(total=12, required=11, pubkeys=pubkeys[:12]),
        )
        T2(
            CScript(la + [15] + pubkeys + [15, OP_CHECKMULTISIG]),
    ...
    def test_repr(self) -> None:
        ...
        T(CScript([]), "CBitcoinScript([])")

        T(CScript([1]), "CBitcoinScript([1])")

        T(CScript([1, 2, 3]), "CBitcoinScript([1, 2, 3])")
        ...
        T(CScript(x("614c")), "CBitcoinScript([OP_NOP, <ERROR: PUSHDATA1: missing data length>])")
    ...
    def test_to_p2sh_scriptPubKey(self) -> None:
        def T(redeemScript_ops: List[Union[int, bytes]], expected_hex_bytes: str) -> None:
