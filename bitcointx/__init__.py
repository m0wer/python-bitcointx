...
class ChainParamsMeta(ABCMeta):
    ...
    def __new__(
        ...
        cls_instance = cast(Type["ChainParamsBase"], super().__new__(cls, cls_name, bases, dct))
        ...
        if len(bases):
            ...
            if name is not None:
                if isinstance(name, str):
                    ...
                    raise TypeError("name argument must be string, list, or tuple")
                for name in names:
...
class ChainParamsBase(metaclass=ChainParamsMeta):
    ...
    PSBT_DISPATCHER: Union[
        ...
        Callable[[object], Type["bitcointx.core.psbt.PSBT_CoinClassDispatcher"]],
    ]
...
class BitcoinMainnetParams(ChainParamsBase, name=("bitcoin", "bitcoin/mainnet")):
    RPC_PORT = 8332
...
@contextmanager
def ChainParams(
    params: Union[str, ChainParamsBase, Type[ChainParamsBase]], **kwargs: Any
) -> Generator[ChainParamsBase, None, None]:
...
def select_chain_params(
    ...
    if isinstance(params, str):
        ...
        if not issubclass(params, ChainParamsBase):
            raise TypeError("supplied class is not a subclass of ChainParamsBase")
        ...
        raise ValueError(
            "Supplied chain params is not a string, not a "
