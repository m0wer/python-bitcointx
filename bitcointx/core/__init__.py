...
T_CoreCoinClassDispatcher = TypeVar("T_CoreCoinClassDispatcher", bound="CoreCoinClassDispatcher")
...
class CoreCoinClassDispatcher(
    ClassMappingDispatcher, identity="core", depends=[script.ScriptCoinClassDispatcher]
):
    def __init_subclass__(mcs: Type[T_CoreCoinClassDispatcher], **kwargs: Any) -> None:
        super().__init_subclass__(**kwargs)

    def __new__(
        mcs: Type[T_CoreCoinClassDispatcher],
        name: str,
        bases: Tuple[type, ...],
        namespace: Dict[str, Any],
        mutable_of: Optional["CoreCoinClassDispatcher"] = None,
        **kwargs: Any,
    ) -> T_CoreCoinClassDispatcher:
        return super().__new__(mcs, name, bases, namespace, **kwargs)

    def __init__(
        cls: T_CoreCoinClassDispatcher,
        name: str,
        bases: Tuple[type, ...],
        namespace: Dict[str, Any],
        ...
        **kwargs: Any,
        ...
        if mutable_of is None:
            if not (cls.__name__ == "CoreCoinClass" or issubclass(cls, CoreCoinClass)):
            ...
            if not issubclass(cls, mutable_of):
                raise TypeError(
                    f"{cls.__class__.__name__} must be a subclass "
    ...
    def __getattribute__(cls, name: str) -> Any:
        if _mutable_context.mutable_context_enabled and name == "_from_instance":
            # In the mutable context, new instances created should be mutable
            cls = type.__getattribute__(cls, "_mutable_cls") or cls
...
class CoreCoinClass(ImmutableSerializable, metaclass=CoreCoinClassDispatcher):
    ...
    @classmethod
    def _from_instance(
        cls: Type[T_CoreCoinClass], other_inst: "CoreCoinClass", *args: Any, **kwargs: Any
    ) -> T_CoreCoinClass:
        ensure_isinstance(other_inst, cls._immutable_cls, "the argument")
    ...
    @classmethod
    @abstractmethod
    def from_instance(cls: Type[T_CoreCoinClass], other_inst: Any) -> T_CoreCoinClass: ...
...
def coins_to_satoshi(value: Union[int, float, decimal.Decimal], check_range: bool = True) -> int:
...
def calculate_transaction_virtual_size(
    *,
    ...
    unscaled_size = base_size * WITNESS_SCALE_FACTOR + witness_size + WITNESS_SCALE_FACTOR - 1
...
class _UintBitVector(ImmutableSerializable, metaclass=_UintBitVectorMeta):
    ...
    def stream_deserialize(
        cls: Type[T__UintBitVector], f: ByteStream_Type, **kwargs: Any
    ) -> T__UintBitVector:
...
class COutPoint(CoreCoinClass, next_dispatch_final=True):
    ...
    def __init__(self, hash: Union[bytes, bytearray] = b"\x00" * 32, n: int = 0xFFFFFFFF):
        ensure_isinstance(hash, (bytes, bytearray), "hash")
        ensure_isinstance(n, int, "n")
        if not len(hash) == 32:
            raise ValueError(
                "%s: hash must be exactly 32 bytes; got %d bytes"
                % (self.__class__.__name__, len(hash))
            )
        object.__setattr__(self, "hash", hash)
        if not (0 <= n <= 0xFFFFFFFF):
            raise ValueError(
                "%s: n must be in range 0x0 to 0xffffffff; got %x" % (self.__class__.__name__, n)
            )
        object.__setattr__(self, "n", n)

    @classmethod
    def stream_deserialize(
        cls: Type[T_COutPoint], f: ByteStream_Type, **kwargs: Any
    ) -> T_COutPoint:
        hash = ser_read(f, 32)
    ...
    def __repr__(self) -> str:
        if self.is_null():
            ...
            return "%s(lx(%r), %i)" % (self.__class__.__name__, b2lx(self.hash), self.n)

    def __str__(self) -> str:
        return "%s:%i" % (b2lx(self.hash), self.n)

    @classmethod
    def from_instance(cls: Type[T_COutPoint], outpoint: "COutPoint") -> T_COutPoint:
        return cls._from_instance(outpoint, outpoint.hash, outpoint.n)

    @classmethod
    def from_outpoint(cls: Type[T_COutPoint], outpoint: "COutPoint") -> T_COutPoint:
        return cls.from_instance(outpoint)


class CMutableOutPoint(COutPoint, mutable_of=COutPoint, next_dispatch_final=True):
...
class CBitcoinOutPoint(COutPoint, CoreBitcoinClass):
    ...
    to_mutable: ClassVar[Callable[["CBitcoinOutPoint"], "CBitcoinMutableOutPoint"]]
    to_immutable: ClassVar[Callable[["CBitcoinOutPoint"], "CBitcoinOutPoint"]]


class CBitcoinMutableOutPoint(CBitcoinOutPoint, CMutableOutPoint, mutable_of=CBitcoinOutPoint):
...
class CTxIn(CoreCoinClass, next_dispatch_final=True):
    ...
    def __init__(
        ...
        if not (0 <= nSequence <= 0xFFFFFFFF):
            raise ValueError(
                "CTxIn: nSequence must be an integer between 0x0 and 0xffffffff; got %x" % nSequence
            )
        if scriptSig is None:
            scriptSig = script.CScript()
        elif not isinstance(scriptSig, script.CScript):
            ensure_isinstance(
                scriptSig, (bytes, bytearray), "scriptSig that is not an instance of CScript"
            )
            scriptSig = script.CScript(scriptSig)
        else:
            ensure_isinstance(
                scriptSig, script.CScript().__class__, "scriptSig that is an instance of CScript"
            )
        if prevout is None:
            prevout = COutPoint()
    ...
    @classmethod
    def stream_deserialize(cls: Type[T_CTxIn], f: ByteStream_Type, **kwargs: Any) -> T_CTxIn:
    ...
    def from_instance(cls: Type[T_CTxIn], txin: "CTxIn") -> T_CTxIn:
        return cls._from_instance(
            txin, COutPoint.from_outpoint(txin.prevout), txin.scriptSig, txin.nSequence
        )
...
class CBitcoinMutableTxIn(
    CBitcoinTxIn,  # type: ignore
    CMutableTxIn,
    mutable_of=CBitcoinTxIn,
):
...
class CTxOut(CoreCoinClass, next_dispatch_final=True):
    ...
    def __init__(self, nValue: int = -1, scriptPubKey: Optional[script.CScript] = None):
        ...
        if scriptPubKey is None:
            ...
            ensure_isinstance(
                scriptPubKey, (bytes, bytearray), "scriptPubKey that is not an instance of CScript"
            )
            scriptPubKey = script.CScript(scriptPubKey)
        else:
            ensure_isinstance(
                scriptPubKey,
                script.CScript().__class__,
    ...
    @classmethod
    def stream_deserialize(cls: Type[T_CTxOut], f: ByteStream_Type, **kwargs: Any) -> T_CTxOut:
    ...
    def __repr__(self) -> str:
        return "%s(%s, %r)" % (
            self.__class__.__name__,
            str_money_value_for_repr(self.nValue),
            self.scriptPubKey,
        )
...
class CBitcoinMutableTxOut(CBitcoinTxOut, CMutableTxOut, mutable_of=CBitcoinTxOut):
...
class CTxInWitness(CoreCoinClass, next_dispatch_final=True):
    ...
    @classmethod
    def stream_deserialize(
        cls: Type[T_CTxInWitness], f: ByteStream_Type, **kwargs: Any
    ) -> T_CTxInWitness:
...
class CMutableTxInWitness(CTxInWitness, mutable_of=CTxInWitness, next_dispatch_final=True):
...
class CBitcoinTxInWitness(CTxInWitness, CoreBitcoinClass):
    ...
    to_mutable: ClassVar[Callable[["CBitcoinTxInWitness"], "CBitcoinMutableTxInWitness"]]
    to_immutable: ClassVar[Callable[["CBitcoinTxInWitness"], "CBitcoinTxInWitness"]]


class CBitcoinMutableTxInWitness(
    CBitcoinTxInWitness, CMutableTxInWitness, mutable_of=CBitcoinTxInWitness
):
...
class CTxOutWitness(CoreCoinClass, next_dispatch_final=True):
    ...
    @classmethod
    def from_instance(cls: Type[T_CTxOutWitness], witness: "CTxOutWitness") -> T_CTxOutWitness:
        return cls._from_instance(witness)


class CMutableTxOutWitness(CTxOutWitness, mutable_of=CTxOutWitness, next_dispatch_final=True):
    pass
...
class _CBitcoinDummyMutableTxOutWitness(
    _CBitcoinDummyTxOutWitness, CMutableTxOutWitness, mutable_of=_CBitcoinDummyTxOutWitness
):
    pass
...
class CTxWitness(CoreCoinClass, next_dispatch_final=True):
    ...
    def __init__(
        self, vtxinwit: Iterable[CTxInWitness] = (), vtxoutwit: Iterable[CTxOutWitness] = ()
    ) -> None:
    ...
    @classmethod
    def stream_deserialize(
        cls: Type[T_CTxWitness], f: ByteStream_Type, num_inputs: int = -1, **kwargs: Any
    ) -> T_CTxWitness:
        if num_inputs < 0:
            raise ValueError("num_inputs must be specified (and must be non-negative)")
        vtxinwit = tuple(
            CTxInWitness.stream_deserialize(f, **kwargs) for dummy in range(num_inputs)
        )
        return cls(vtxinwit)
    ...
    @classmethod
    def from_instance(cls: Type[T_CTxWitness], witness: "CTxWitness") -> T_CTxWitness:
        vtxinwit = (CTxInWitness.from_txin_witness(w) for w in witness.vtxinwit)
        return cls._from_instance(witness, vtxinwit)

    @classmethod
    def from_witness(cls: Type[T_CTxWitness], witness: "CTxWitness") -> T_CTxWitness:
        return cls.from_instance(witness)

    def __repr__(self) -> str:
        return "%s([%s])" % (self.__class__.__name__, ",".join(repr(w) for w in self.vtxinwit))


class CMutableTxWitness(CTxWitness, mutable_of=CTxWitness, next_dispatch_final=True):
...
class CBitcoinTxWitness(CTxWitness, CoreBitcoinClass):
    ...
    to_mutable: ClassVar[Callable[["CBitcoinTxWitness"], "CBitcoinMutableTxWitness"]]
    to_immutable: ClassVar[Callable[["CBitcoinTxWitness"], "CBitcoinTxWitness"]]


class CBitcoinMutableTxWitness(CBitcoinTxWitness, CMutableTxWitness, mutable_of=CBitcoinTxWitness):
...
class CTransaction(ReprOrStrMixin, CoreCoinClass, next_dispatch_final=True):
    ...
    def __init__(
        self,
        vin: Iterable[CTxIn] = (),
        vout: Iterable[CTxOut] = (),
        nLockTime: int = 0,
        nVersion: Optional[int] = None,
        ...
        if not (0 <= nLockTime <= 0xFFFFFFFF):
            raise ValueError(
                "CTransaction: nLockTime must be in range 0x0 to 0xffffffff; got %x" % nLockTime
            )
        ...
        if witness is None or (witness.is_null() and self.is_mutable()):
            if self.is_mutable():
                new_witness = CTxWitness(
                    [CTxInWitness() for dummy in vin], [CTxOutWitness() for dummy in vout]
                )
                if witness is not None:
                    ensure_isinstance(witness, new_witness._immutable_cls, "witness")
                witness = new_witness
            else:
        ...
        object.__setattr__(self, "vin", tuple_or_list(CTxIn.from_txin(txin) for txin in vin))
        object.__setattr__(self, "vout", tuple_or_list(CTxOut.from_txout(txout) for txout in vout))
    ...
    def GetTxid(self) -> bytes:
        ...
        if not self.wit.is_null():
            txid = Hash(
                CTransaction(self.vin, self.vout, self.nLockTime, self.nVersion).serialize()
            )
        else:
    ...
    @classmethod
    def from_instance(cls: Type[T_CTransaction], tx: "CTransaction") -> T_CTransaction:
        vin = [CTxIn.from_txin(txin) for txin in tx.vin]
        vout = [CTxOut.from_txout(txout) for txout in tx.vout]
        wit = CTxWitness.from_witness(tx.wit)
        return cls._from_instance(tx, vin, vout, tx.nLockTime, tx.nVersion, wit)

    @classmethod
    def from_tx(cls: Type[T_CTransaction], tx: "CTransaction") -> T_CTransaction:
        return cls.from_instance(tx)

    @classmethod
    def stream_deserialize(
        cls: Type[T_CTransaction], f: ByteStream_Type, **kwargs: Any
    ) -> T_CTransaction:
        ...
        vin = VectorSerializer.stream_deserialize(f, element_class=CTxIn, **kwargs)
        ...
        if not vin:
            ...
            if flags != 0:
                vin = VectorSerializer.stream_deserialize(f, element_class=CTxIn, **kwargs)
                vout = VectorSerializer.stream_deserialize(f, element_class=CTxOut, **kwargs)
        else:
        ...
        if flags & 1:
            ...
            wit = CTxWitness.stream_deserialize(f, num_inputs=len(vin), **kwargs)
            if wit.is_null():
    ...
    def stream_serialize(
        self, f: ByteStream_Type, include_witness: bool = True, **kwargs: Any
    ) -> None:
...
class CMutableTransaction(CTransaction, mutable_of=CTransaction, next_dispatch_final=True):
...
class CBitcoinTransaction(CTransaction, CoreBitcoinClass):
    ...
    to_mutable: ClassVar[Callable[["CBitcoinTransaction"], "CBitcoinMutableTransaction"]]
    to_immutable: ClassVar[Callable[["CBitcoinTransaction"], "CBitcoinTransaction"]]


class CBitcoinMutableTransaction(
    CBitcoinTransaction, CMutableTransaction, mutable_of=CBitcoinTransaction
):
...
def CheckTransaction(tx: CTransaction) -> None:  # noqa
    ...
    weight = len(base_tx.serialize(include_witness=False)) * CoreCoinParams.WITNESS_SCALE_FACTOR
