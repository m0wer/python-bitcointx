...
class CKeyBase:
    ...
    def __init__(self, b: Optional[bytes], compressed: bool = True) -> None:
        ...
        result = secp256k1.lib.secp256k1_ec_seckey_verify(secp256k1.ctx.sign, self.secret_bytes)
        ...
        self.__pub = CPubKey._from_ctypes_char_array(raw_pubkey, compressed=compressed)
        ...
        *,
        ...
        while True:
            result = secp256k1.lib.secp256k1_ecdsa_sign(
                secp256k1.ctx.sign, raw_sig, hash, self.secret_bytes, None, maybe_extra_entropy()
            )
            if 1 != result:
                assert result == 0
    ...
    def sign_compact(self, hash: Union[bytes, bytearray]) -> Tuple[bytes, int]:  # pylint: disable=redefined-builtin
        ...
        if 1 != result:
            assert result == 0
            raise RuntimeError(
                "secp256k1_ecdsa_recoverable_signature_serialize_compact returned failure"
            )

        return output.raw, recid.value

    def sign_schnorr_no_tweak(
        self, hash: Union[bytes, bytearray], *, aux: Optional[bytes] = None
    ) -> bytes:
        """
    ...
    def _sign_schnorr_internal(  # noqa
        ...
        *,
        ...
        if not secp256k1.cap.has_schnorrsig:
            raise RuntimeError(_module_unavailable_error("schnorr signature", "schnorrsig"))
        ...
        result = secp256k1.lib.secp256k1_keypair_create(secp256k1.ctx.sign, keypair_buf, self)
        ...
        if merkle_root is not None:
            ...
            tweak = compute_tap_tweak_hash(
                XOnlyPubKey(serialized_pubkey_buf.raw), merkle_root=merkle_root
            )
        ...
        if result != 1:
            assert result == 0
            raise RuntimeError("secp256k1_schnorrsig_verify failed after signing")
    ...
    def ECDH(self, pub: Optional["CPubKey"] = None) -> bytes:
        ...
        ret = secp256k1.lib.secp256k1_ecdh(
            secp256k1.ctx.sign, result_data, pub._to_ctypes_char_array(), self, None, None
        )
        if 1 != ret:
            assert ret == 0
    ...
    @classmethod
    def combine(
        cls: Type[T_CKeyBase], *privkeys: T_CKeyBase, compressed: bool = True
    ) -> T_CKeyBase:
        if len(privkeys) <= 1:
            raise ValueError("number of privkeys to combine must be more than one")
        if not all(isinstance(k, CKeyBase) for k in privkeys):
            raise ValueError("each supplied privkey must be an instance of CKeyBase")
    ...
    def add(cls: Type[T_CKeyBase], a: T_CKeyBase, b: T_CKeyBase) -> T_CKeyBase:
        if a.is_compressed() != b.is_compressed():
            raise ValueError("compressed attributes must match on privkey addition/substraction")
        return cls.combine(a, b, compressed=a.is_compressed())

    @classmethod
    def sub(cls: Type[T_CKeyBase], a: T_CKeyBase, b: T_CKeyBase) -> T_CKeyBase:
        if a == b:
            raise ValueError("Values are equal, result would be zero, and thus an invalid key.")
    ...
    def from_secret_bytes(
        cls: Type[T_CKeyBase], secret: bytes, compressed: bool = True
    ) -> T_CKeyBase:
...
class CKey(bytes, CKeyBase):
    ...
    def __new__(cls: Type["CKey"], secret: bytes, compressed: bool = True) -> "CKey":
        if len(secret) != 32:
...
class CPubKey(bytes):
    ...
    @classmethod
    def _from_ctypes_char_array(
        cls: Type[T_CPubKey], raw_pubkey: "ctypes.Array[ctypes.c_char]", compressed: bool = True
    ) -> T_CPubKey:
    ...
    @classmethod
    def recover_compact(cls: Type[T_CPubKey], hash: bytes, sig: bytes) -> Optional[T_CPubKey]:
        """Recover a public key from a compact signature."""
        if len(sig) != COMPACT_SIGNATURE_SIZE:
            raise ValueError(
                "Signature should be %d characters, not [%d]" % (COMPACT_SIGNATURE_SIZE, len(sig))
            )
    ...
    def verify(self, hash: bytes, sig: bytes) -> bool:
        ...
        secp256k1.lib.secp256k1_ecdsa_signature_normalize(secp256k1.ctx.verify, raw_sig, raw_sig)

        raw_pub = self._to_ctypes_char_array()
        result = secp256k1.lib.secp256k1_ecdsa_verify(secp256k1.ctx.verify, raw_sig, hash, raw_pub)
    ...
    @classmethod
    def combine(cls: Type[T_CPubKey], *pubkeys: T_CPubKey, compressed: bool = True) -> T_CPubKey:
        if len(pubkeys) <= 1:
            raise ValueError("number of pubkeys to combine must be more than one")
        for p in pubkeys:
            if not isinstance(p, CPubKey):
                raise ValueError("each supplied pubkey must be an instance of CPubKey")
    ...
    def negated(self: T_CPubKey) -> T_CPubKey:
        ...
        return self.__class__._from_ctypes_char_array(pubkey_buf, compressed=self.is_compressed())

    @classmethod
    def add(cls: Type[T_CPubKey], a: T_CPubKey, b: T_CPubKey) -> T_CPubKey:
        if a.is_compressed() != b.is_compressed():
            raise ValueError("compressed attributes must match on pubkey addition/substraction")
        return cls.combine(a, b, compressed=a.is_compressed())
    ...
    def sub(cls: Type[T_CPubKey], a: T_CPubKey, b: T_CPubKey) -> T_CPubKey:
        if a == b:
            raise ValueError(
                "Values are equal, result would be zero, and thus an invalid public key."
            )
        return cls.add(a, b.negated())
...
class CExtKeyCommonBase:
    ...
    def _check_depth(self) -> None:
        if self.depth == 0:
            ...
            if self.child_number != 0:
                raise ValueError(
                    "Derivation depth of the key is 0, the child_number must be 0, but it is not"
                )
    ...
    def derive_path(
        self: T_CExtKeyCommonBase, path: Union[str, "BIP32Path", Sequence[int]]
    ) -> T_CExtKeyCommonBase:
        ...
        if not path.is_partial() and self.depth != 0:
            raise ValueError(
                "full derivation path was supplied, but this extended (pub)key has depth != 0"
            )
    ...
    def assign_derivation_info(self, derivation_info: Optional["KeyDerivationInfo"]) -> None:
        try:
            if self.__derivation_info:
                raise AttributeError("derivation info already present for this instance")
        ...
        if derivation_info:
            ...
            if self.depth == 1 and self.parent_fp != derivation_info.master_fp:
                raise ValueError(
                    "master fingerprint in derivation info is not "
                    "the same as extended key parent fingerprint while "
                    "this key has depth 1"
                )
            elif self.depth == 0 and self.fingerprint != derivation_info.master_fp:
                raise ValueError(
                    "master fingerprint in derivation info is not "
            ...
            if self.depth == 0:
                ...
                derivation_info = KeyDerivationInfo(
                    self.parent_fp, BIP32Path([self.child_number], is_partial=False)
                )
...
class CExtKeyBase(CExtKeyCommonBase):
    ...
    def from_seed(cls: Type[T_CExtKeyBase], seed: bytes) -> T_CExtKeyBase:
        ...
        return cls.from_bytes(
            bytes([depth]) + parent_fp + child_number_packed + chaincode + bytes([0]) + privkey
        )
    ...
    def derive(self: T_CExtKeyBase, child_number: int) -> T_CExtKeyBase:
        ...
        if (child_number >> 31) == 0:
            bip32_hash = hmac.new(
                self.chaincode, self.pub + child_number_packed, hashlib.sha512
            ).digest()
        else:
            bip32_hash = hmac.new(
                self.chaincode,
        ...
        if self.derivation_info:
            instance.assign_derivation_info(
                KeyDerivationInfo(
                    self.derivation_info.master_fp, self.derivation_info.path + [child_number]
                )
            )
...
class CExtPubKeyBase(CExtKeyCommonBase):
    ...
    def derive(self: T_CExtPubKeyBase, child_number: int) -> T_CExtPubKeyBase:
        ...
        result = secp256k1.lib.secp256k1_ec_pubkey_serialize(
            secp256k1.ctx.verify,
            child_pubkey,
            ctypes.byref(child_pubkey_size0),
            raw_pub,
            SECP256K1_EC_COMPRESSED,
        )
        ...
        cls = self.__class__
        instance = cls.from_bytes(
            bytes([depth]) + self.fingerprint + child_number_packed + chaincode + child_pubkey.raw
        )
        if self.derivation_info:
            instance.assign_derivation_info(
                KeyDerivationInfo(
...
class BIP32PathGeneric(Generic[T_BIP32PathIndex]):
    ...
    def __init__(  # noqa
        ...
        path: Union[
            str,
            "BIP32PathGeneric[T_BIP32PathIndex]",  # noqa
        ...
        self._indexes = tuple(self.__class__._index_from_argument(n) for n in indexes)
    ...
    def __add__(
        self, other: Union["BIP32PathGeneric[T_BIP32PathIndex]", Iterable[T_BIP32PathIndex]]
    ) -> "BIP32PathGeneric[T_BIP32PathIndex]":
        if isinstance(other, BIP32PathGeneric):
            if not other.is_partial():
                raise ValueError(
                    "cannot append full path to anything, can only append partial path to full path"
                )
        return self.__class__(list(self._indexes) + list(other), is_partial=self.is_partial())
    ...
    def _parse_string(
        self,
        path: str,
        hardened_marker: Optional[str] = None,  # noqa
        ...
        if path == "":
            ...
            if path.startswith("/"):
                raise ValueError('partial derivation path must not start with "/"')
            is_partial = True
        ...
        for pos, elt in enumerate(path.split("/")):
            if elt == "":
                if path == "":
                    raise ValueError(
                        'empty non-partial path must be specified as "m" (without slash)'
                    )
            ...
            if c[-1] in self.__class__.HARDENED_MARKERS:
                if expected_marker is None:
                    ...
                    raise ValueError(
                        'Unexpected hardened marker: "{}" {}, but got {}'.format(
                            expected_marker,
                            (
            ...
            indexes.append(self.__class__._index_from_str(c, is_hardened=is_hardened))
...
class BIP32Path(BIP32PathGeneric[int]):
    ...
    def _index_from_str(cls, s: str, *, is_hardened: bool) -> int:
        ...
        if n >= BIP32_HARDENED_KEY_OFFSET:
            raise ValueError(
                f"derivation index string cannot represent value > {BIP32_HARDENED_KEY_OFFSET - 1}"
            )
    ...
    def __add__(self, other: Union["BIP32Path", Iterable[int]]) -> "BIP32Path":
    ...
    def __init__(self, index_tuples: Iterable[Tuple[int, int]]) -> None:
        ...
        for from_to_tuple in index_tuples:
            ...
            if len(from_to_tuple) != 2:
                raise ValueError("index tuple must have two values: from and to")
            ...
            if left_hardened != right_hardened:
                raise ValueError("index bounds must be both hardened or both unhardened")

            if left > right:
                raise ValueError("index_from cannot be larger than index_to in an index tuple")
...
class BIP32PathTemplate(BIP32PathGeneric[BIP32PathTemplateIndex]):
    ...
    def _index_from_str(
        ...
        *,
        ...
        def parse_index(s: str, *, is_hardened: bool) -> Tuple[Optional[int], Optional[ValueError]]:
            try:
        ...
        if index_str == "*":
            return BIP32PathTemplateIndex([(offset, BIP32_HARDENED_KEY_OFFSET - 1 + offset)])
        ...
        for index_substr in index_str.split(","):
            ...
            if len(maybe_range) > 1:
                ...
                if n_left == 0 and n_right == BIP32_HARDENED_KEY_OFFSET - 1:
                    raise ValueError(
                        'index range equals wildcard range, should be specified as "*"'
                    )
            ...
            if index_bounds_list and index_bounds_list[-1][1] + 1 == range_tuple[0]:
                index_bounds_list[-1] = (index_bounds_list[-1][0], range_tuple[1])
            else:
    ...
    def _index_to_str(self, ti: BIP32PathTemplateIndex) -> str:
        ...
        for left, right in ti:
            ...
            right_hardened = right >= BIP32_HARDENED_KEY_OFFSET
            if left_hardened != right_hardened:
                raise AssertionError("index bounds must be both hardened or both unhardened")
    ...
    def __add__(
        self, other: Union["BIP32PathTemplate", Iterable[BIP32PathTemplateIndex]]
    ) -> "BIP32PathTemplate":
...
class KeyDerivationInfo:
    ...
    def __init__(self, master_fp: bytes, path: BIP32Path) -> None:
    ...
    def clone(self: T_KeyDerivationInfo) -> T_KeyDerivationInfo:
        return self.__class__(master_fp=self.master_fp, path=BIP32Path(path=self.path))


T_KeyStoreKeyArg = Union[
    CKeyBase,
    CPubKey,
    CExtKeyBase,
    CExtPubKeyBase,
    Tuple[CExtKeyBase, Union[BIP32PathTemplate, Iterable[BIP32PathTemplate], str, Iterable[str]]],
    Tuple[
        CExtPubKeyBase, Union[BIP32PathTemplate, Iterable[BIP32PathTemplate], str, Iterable[str]]
    ],
]
...
class KeyStore:
    ...
    _external_privkey_lookup: Optional[
        Callable[[bytes, Optional[KeyDerivationInfo]], Optional[CKeyBase]]
    ]
    _external_pubkey_lookup: Optional[
        Callable[[bytes, Optional[KeyDerivationInfo]], Optional[CPubKey]]
    ]
    ...
    def __init__(
        ...
        *args: T_KeyStoreKeyArg,
        ...
        default_path_template: Optional[Union[BIP32PathTemplate, str]] = None,
        ...
        if default_path_template is not None:
            ensure_isinstance(
                default_path_template, (BIP32PathTemplate, str), "default_path_template"
            )
            ...
            if default_path_template.is_partial():
                raise ValueError("default path template must specify full path")
    ...
    ) -> Optional[Callable[[bytes, Optional[KeyDerivationInfo]], Optional[CKeyBase]]]:
    ...
    ) -> Optional[Callable[[bytes, Optional[KeyDerivationInfo]], Optional[CPubKey]]]:
    ...
    @classmethod
    def from_iterable(
        cls: Type[T_KeyStore], iterable: Iterable[T_KeyStoreKeyArg], **kwargs: Any
    ) -> T_KeyStore:
        kstore = cls(**kwargs)
        for k in iterable:
            kstore.add_key(k)
    ...
    def add_key(self, k: T_KeyStoreKeyArg) -> None:  # noqa
        ...
        if isinstance(k, tuple):
            ...
            if isinstance(pts, (str, BIP32PathTemplate)):
                ...
                for pt in pts:
                    ensure_isinstance(pt, (str, BIP32PathTemplate), "path template")
                    path_templates.append(BIP32PathTemplate(pt))

                # Note: cannot do `if not pts` because it might be a generator
                if not path_templates:
                    raise ValueError("path templates list is empty")

        if isinstance(k, CKeyBase):
            if path_templates:
                raise ValueError("path_templates only make sense for extended keys")
            ...
            if not path_templates and self._require_path_templates:
                raise ValueError("path templates must be specified for extended key")

            for pt in path_templates:
                ensure_isinstance(pt, BIP32PathTemplate, "path template")

            mfp, indexes = self._mfp_and_indexes_from_derivation(k.derivation_info)
    ...
    def _enforce_path_templates(
        ...
        if not path_templates:
            if self._require_path_templates:
                raise AssertionError(
                    "path_templates cannot be empty when require_path_templates is set"
                )
            return
        ...
        if full_path is None and partial_path is None:
            raise ValueError(
                "at least one of full_path_indexes or partial_path_indexes must be specified"
            )
        ...
        if master_fp in l0_dict:
            ...
            for l1_indexes, xpub_dict in l1_dict.items():
                if indexes[: len(l1_indexes)] == l1_indexes:
                    ...
                    if all(idx < BIP32_HARDENED_KEY_OFFSET for idx in indexes_tail):
    ...
    def get_privkey(
        self, key_id: bytes, derivation: Optional[T_KeyDerivationInfo] = None
    ) -> Optional[CKeyBase]:
...
class XOnlyPubKey(bytes):
    ...
    def __new__(cls: Type[T_XOnlyPubKey], keydata: Union[bytes, CPubKey] = b"") -> T_XOnlyPubKey:
        ...
        if len(keydata) in (32, 0):
            ...
            ensure_isinstance(keydata, CPubKey, "x-only pubkey data of 33 bytes")
        ...
        if self.is_nonempty():
            ...
            result = secp256k1.lib.secp256k1_xonly_pubkey_parse(secp256k1.ctx.verify, tmpbuf, self)
            assert result in (1, 0)
            self.__fullyvalid = result == 1
    ...
    def verify_schnorr(self, hash: bytes, sigbytes: bytes) -> bool:
        ...
        result = secp256k1.lib.secp256k1_schnorrsig_verify(
            secp256k1.ctx.verify, sigbytes, hash, 32, self._to_ctypes_char_array()
        )
    ...
    def _to_ctypes_char_array(self) -> "ctypes.Array[ctypes.c_char]":
        ...
        secp256k1 = get_secp256k1()
        raw_pub = ctypes.create_string_buffer(64)
        result = secp256k1.lib.secp256k1_xonly_pubkey_parse(secp256k1.ctx.verify, raw_pub, self)
...
def compute_tap_tweak_hash(pub: XOnlyPubKey, *, merkle_root: bytes = b"") -> bytes:
    ...
    return bitcointx.core.CoreCoinParams.taptweak_hasher(pub + merkle_root)
...
def check_tap_tweak(
    ...
    result = secp256k1.lib.secp256k1_xonly_pubkey_tweak_add_check(
        ...
        tweaked_pub,
        int(bool(parity)),
        internal_pub._to_ctypes_char_array(),
        tweak,
    )
...
def tap_tweak_pubkey(
    ...
    *,
    ...
    result = secp256k1.lib.secp256k1_xonly_pubkey_from_pubkey(
        secp256k1.ctx.verify, out_xonly, ctypes.byref(parity_ret), out
    )
