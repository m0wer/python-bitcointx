...
class PSBT_CoinClassDispatcher(
    ClassMappingDispatcher, identity="psbt", depends=[bitcointx.core.CoreCoinClassDispatcher]
): ...
...
class PSBT_BitcoinClassDispatcher(
    PSBT_CoinClassDispatcher, depends=[bitcointx.core.CoreBitcoinClassDispatcher]
): ...
...
def proprietary_field_repr(prop_fields_dict: Dict[bytes, List[PSBT_ProprietaryTypeData]]) -> str:
    def prop_str(p_list: List[PSBT_ProprietaryTypeData]) -> str:
        return ", ".join(
            f"({v.subtype}, x('{b2x(v.key_data)}'), x('{b2x(v.value)}'))" for v in p_list
        )

    return ", ".join(f"x('{b2x(k)}'): ({prop_str(v)})" for k, v in prop_fields_dict.items())


def unknown_fields_repr(unknown_fields: List[PSBT_UnknownTypeData]) -> str:
    return ", ".join(
        f"({v.key_type}, x('{b2x(v.key_data)}'), x('{b2x(v.value)}'))" for v in unknown_fields
    )


def derivation_map_repr(derivation_map: Dict[CPubKey, "PSBT_KeyDerivationInfo"]) -> str:
    return ", ".join(
        f"x('{b2x(k or b'')}'): (x('{b2x(v.master_fp)}'), \"{str(v.path)}\")"
        for k, v in derivation_map.items()
    )


def merge_input_output_common_fields(
    dst: Union["PSBT_Input", "PSBT_Output"], src: Union["PSBT_Input", "PSBT_Output"], what: str
) -> None:
    ...
    if not dst.redeem_script:
        ...
        if dst.redeem_script != src.redeem_script:
            raise ValueError(f"redeem scripts are different for {what}sat index {dst.index}")
    ...
    for pub, dinfo in src.derivation_map.items():
        if pub in dst.derivation_map:
            ...
            assert dst.derivation_map[pub] == dinfo, "if key_ids match, pubkeys must match"
        else:
    ...
    for prefix in proprietary_fields.keys():
        for prop_data in proprietary_fields[prefix]:
            prop_key = (
                ...
                + VarIntSerializer.serialize(prop_data.subtype, allow_full_range=True)
                + prop_data.key_data
            )
            stream_serialize_field(
                PSBT_PROPRIETARY_TYPE, f, key_data=prop_key, value=prop_data.value
            )
    ...
    for prefix, propdata_list in proprietary_fields_src.items():
        ...
        for pd in propdata_list:
            ...
            proprietary_fields_dst[prefix].append(
                PSBT_ProprietaryTypeData(subtype=pd.subtype, key_data=pd.key_data, value=pd.value)
            )
...
def stream_serialize_unknown_fields(
    ...
    for unk_data in unknown_fields:
        stream_serialize_field(
            unk_data.key_type, f, key_data=unk_data.key_data, value=unk_data.value
        )
...
def merge_unknown_fields(
    ...
    for ud in unknown_fields_src:
        ...
        unknown_fields_dst.append(
            PSBT_UnknownTypeData(key_type=ud.key_type, key_data=ud.key_data, value=ud.value)
        )
...
def read_psbt_keymap(
    ...
    while True:
        ...
        if key_data in keys_seen:
            ...
            raise SerializationError(f"Duplicate key encountered at position {tellf()}")

        keys_seen.add(key_data)

        key_type, key_data = VarIntSerializer.deserialize_partial(key_data, allow_full_range=True)
        ...
        if key_type == PSBT_PROPRIETARY_TYPE:
            ...
            subtype, key_data = VarIntSerializer.deserialize_partial(tail, allow_full_range=True)
            field = PSBT_ProprietaryTypeData(subtype=subtype, key_data=key_data, value=value)
        ...
        try:
            ...
            unknown_fields.append(
                PSBT_UnknownTypeData(key_type=key_type, key_data=key_data, value=value)
            )
            continue
...
def ensure_empty_key_data(key_type: T_KeyTypeEnum, key_data: bytes, msg_suffix: str = "") -> None:
    if key_data:
        raise SerializationError(f"Unexpected data after key type {key_type.name}" + msg_suffix)


T_PSBT_KeyDerivationInfo = TypeVar("T_PSBT_KeyDerivationInfo", bound="PSBT_KeyDerivationInfo")


class PSBT_KeyDerivationInfo(ImmutableSerializable, KeyDerivationInfo):
    @classmethod
    def stream_deserialize(
        cls: Type[T_PSBT_KeyDerivationInfo],
        f: ByteStream_Type,
        _err_msg_suffix: str = "",
        **kwargs: Any,
        ...
        while True:
            ...
            if len(indexlist) > 255:
                raise ValueError("Derivation path longer than 255 elements" + _err_msg_suffix)
...
class PSBT_Input(PSBT_CoinClass, next_dispatch_final=True):
    ...
    def __init__(
        ...
        *,
        ...
        proprietary_fields: Optional[Dict[bytes, List[PSBT_ProprietaryTypeData]]] = None,
        ...
        if index is not None:
            ...
            if unsigned_tx and index >= len(unsigned_tx.vin):
                raise ValueError("index is beyond length of inputs of unsigned_tx")
        ...
        for pub, sig in partial_sigs.items():
            ensure_isinstance(pub, CPubKey, descr("pubkey for one of the partial signatures"))
            ensure_isinstance(sig, bytes, descr("one of the partial signatures"))
        ...
        for pub, derinfo in derivation_map.items():
            ensure_isinstance(pub, CPubKey, descr("one of pubkeys in bip32 derivation map"))
            ensure_isinstance(
                derinfo,
        ...
        if final_script_witness is None:
            ...
            ensure_isinstance(final_script_witness, CScriptWitness, descr("final script witness"))

        self.final_script_witness = final_script_witness

        ensure_isinstance(
            proof_of_reserves_commitment, bytes, descr("proof of reserves commitment")
        )
        ...
        for prefix, p_fields in proprietary_fields.items():
            ...
            for field_index, field in enumerate(p_fields):
                ensure_isinstance(
                    ...
                    descr(f"one of proprietary field contents for prefix {b2x(prefix)}"),
                )
        ...
        for u_field in unknown_fields:
            ensure_isinstance(u_field, PSBT_UnknownTypeData, descr("contents of unkown type"))
    ...
    def set_utxo(
        ...
        input_descr = "input" if self.index is None else f"input at index {self.index}"
        ...
        def check_witness_utxo_spk(spk: CScript) -> bool:
            ...
            if spk.is_witness_scriptpubkey():
                if self.redeem_script and not relaxed_sanity_checks:
                    raise ValueError(
                        f"redeem script is specified for {input_descr} with non-p2sh segwit prevout"
                    )
                return True
            elif rds:
                if spk.is_p2sh():
                    if rds.is_witness_scriptpubkey():
                        if rds.to_p2sh_scriptPubKey() != spk and not relaxed_sanity_checks:
                            raise ValueError(
                    ...
                    raise ValueError(
                        f"{input_descr} has redeem script specified, but has non-p2sh scriptPubKey"
                    )
                else:
                    return False
            elif spk.is_p2sh():
        ...
        if self.index is None or unsigned_tx is None:
            if must_be_witness_utxo:
                what_is_missing = "unsigned_tx" if unsigned_tx is None else "input index"
                raise ValueError(
        ...
        if txin.prevout.n >= len(utxo.vout):
            raise ValueError(
                f"{input_descr} prevout index in unsigned_tx is beyond the length of utxo.vout"
            )
    ...
    @classmethod
    def from_instance(cls: Type[T_PSBT_Input], inst: T_PSBT_Input) -> T_PSBT_Input:
        new_inst = cls()
    ...
    def merge(self: T_PSBT_Input, other: T_PSBT_Input, allow_blob_duplicates: bool = False) -> None:
        ...
        if not self.utxo:
            ...
            if self.utxo.GetTxid() != other.utxo.GetTxid():
                raise ValueError(
                    f"inputs at index {self.index} have non-witness utxos with different txids"
                )
            ...
            if self.witness_utxo.serialize() != other.witness_utxo.serialize():
                raise ValueError(f"witness utxos are different for inputs at index {self.index}")
            if isinstance(self.utxo, CTxOut) and isinstance(other.utxo, CTransaction):
                ...
                assert isinstance(self.utxo, CTransaction) or isinstance(other.utxo, CTxOut)
        else:
        ...
        if self.sighash_type is None:
            ...
            if self.sighash_type != other.sighash_type:
                raise ValueError(f"sighash types are different for inputsat index {self.index}")
        ...
        if not self.proof_of_reserves_commitment:
            self.proof_of_reserves_commitment = other.proof_of_reserves_commitment
        ...
        merge_unknown_fields(
            self.unknown_fields, other.unknown_fields, allow_duplicates=allow_blob_duplicates
        )
    ...
    def _got_single_sig(
        ...
        *,
        ...
        if self.final_script_sig or self.final_script_witness:
            raise AssertionError(f"trying to sign already finalized input at index {self.index}")
        if finalize:
        ...
        return PSBT_InputSignInfo(num_new_sigs=1, num_sigs_missing=0, is_final=finalize)
        ...
        if is_ready:
            if finalize:
                if is_witness:
                    assert script_sig_for_witness is not None
                    self.final_script_sig = script_sig_for_witness
                    self.final_script_witness = CScriptWitness(
                        msig_helper.construct_witness_stack()
                    )
                else:
                    self.final_script_sig = CScript(msig_helper.construct_witness_stack())
                self._clear_nonfinal_fields()
            elif new_sigs:
                self.partial_sigs.update(new_sigs)

            return PSBT_InputSignInfo(
                num_new_sigs=len(new_sigs), num_sigs_missing=0, is_final=finalize
            )
    ...
    def _get_derinfo_by_key_id(self, key_id: bytes) -> Optional["PSBT_KeyDerivationInfo"]:
    ...
    def is_final(self) -> bool:
        ...
        if self.final_script_witness:
            if not isinstance(utxo, CTxOut):
                inp_descr = "input without utxo specified" if utxo is None else "non-segwit input"
                raise ValueError(f"final_script_witness is present for {inp_descr}")
            self._check_nonfinal_fields_empty()
            if self.final_script_sig:
                if utxo.scriptPubKey.is_witness_scriptpubkey():
                    raise ValueError("final_script_sig is present for native segwit input")
            elif utxo.scriptPubKey.is_p2sh():
                raise ValueError("final_script_sig is not present for p2sh-wrapped segwit input")
            return True
    ...
    def sign(
        ...
        *,
        ...
        assert self.sighash_type != 0, "unspecified sighash_type must be represented by None"

        if self.is_final():
            return PSBT_InputSignInfo(num_new_sigs=0, num_sigs_missing=0, is_final=True)
        ...
        if isinstance(utxo, CTxOut):  # witness UTXO
            if not MoneyRange(utxo.nValue):
                raise ValueError(
                    f"prevout for input at index {self.index} has value out of valid range"
                )
            ...
            if spk.is_witness_scriptpubkey():
                ...
                if rds:
                    raise ValueError(
                        f"redeem script is specified for {input_descr} input at index {self.index}"
                    )
                script_sig = CScript()
                s = spk
            ...
            if s.is_witness_v0_keyhash():
                ...
                sighash = calc_sighash(standard_keyhash_scriptpubkey(s.pubkey_hash()))
                ...
                if self.partial_sigs:
                    ...
                    return self._got_single_sig(
                        pub,
                        self.partial_sigs[pub],
                        is_witness=True,
                        script_sig_for_witness=script_sig,
                        finalize=finalize,
                    )
                ...
                return PSBT_InputSignInfo(num_new_sigs=0, num_sigs_missing=1, is_final=False)
            ...
            if ws:
                raise ValueError(
                    f"witness script is specified for non-segwit input at index {self.index}"
                )

            prevout_index = unsigned_tx.vin[self.index].prevout.n

            if prevout_index >= len(utxo.vout):
                raise ValueError("prevout index in unsigned_tx is beyond the length of utxo.vout")
            ...
            if spk.is_witness_scriptpubkey():
                raise ValueError(
                    f"witness scritpubkey is found for non-witness UTXO at index {self.index}"
                )

            def calc_sighash(script_for_sighash: CScript) -> bytes:
                assert self.index is not None
                return script_for_sighash.sighash(
                    unsigned_tx, self.index, SIGHASH_Type(sighash_type), sigversion=SIGVERSION_BASE
                )

            if spk.is_p2pkh():
                if rds:
                    raise ValueError(
                        f"redeem script is specified for p2pkh input at index {self.index}"
                    )
                ...
                if self.partial_sigs:
                    ...
                    return self._got_single_sig(
                        pub, self.partial_sigs[pub], is_witness=False, finalize=finalize
                    )
                ...
                if key:
                    sig = key.sign(sighash) + bytes([sighash_type])
                    return self._got_single_sig(key.pub, sig, is_witness=False, finalize=finalize)
                return PSBT_InputSignInfo(num_new_sigs=0, num_sigs_missing=1, is_final=False)
            elif spk.is_p2sh():
                if not rds:
                    return None  # redeem script is not specified, cannot sign.
    ...
    def stream_deserialize(
        ...
        **kwargs: Any,
        ...
        proof_of_reserves_commitment: bytes = b""
        proprietary_fields: Dict[bytes, List[PSBT_ProprietaryTypeData]] = OrderedDict()
        unknown_fields: List[PSBT_UnknownTypeData] = []
        ...
        for key_type, key_data, value in read_psbt_keymap(
            f, keys_seen, PSBT_InKeyType, proprietary_fields, unknown_fields
        ):
            if key_type is PSBT_InKeyType.NON_WITNESS_UTXO:
                ...
                if witness_utxo is not None:
                    check_witness_and_nonwitness_utxo_in_sync(witness_utxo, non_witness_utxo)
                ...
                ensure_empty_key_data(key_type, key_data, descr(""))
                if len(value) != 4:
                    raise SerializationError(descr(f"Incorrect data length for {key_type.name}"))
        ...
        return cls(
            utxo=non_witness_utxo or witness_utxo,
            ...
            index=index,
            force_witness_utxo=bool(witness_utxo),
            **kwargs,
        )

    def stream_serialize(
        self, f: ByteStream_Type, always_include_witness_utxo: bool = True, **kwargs: Any
    ) -> None:
        if self.utxo is not None:
            if isinstance(self.utxo, CTransaction):
                assert not self.utxo.is_null()
                stream_serialize_field(
                    PSBT_InKeyType.NON_WITNESS_UTXO, f, value=self.utxo.serialize()
                )
            ...
            if self.witness_utxo and include_witness_utxo:
                ...
                stream_serialize_field(
                    PSBT_InKeyType.WITNESS_UTXO, f, value=self.witness_utxo.serialize()
                )

        if not self.final_script_sig and not self.final_script_witness:
            for pub, sig in self.partial_sigs.items():
                stream_serialize_field(PSBT_InKeyType.PARTIAL_SIG, f, key_data=pub, value=sig)

            if self.sighash_type is not None:
                stream_serialize_field(
                    PSBT_InKeyType.SIGHASH_TYPE, f, value=struct.pack(b"<I", self.sighash_type)
                )

            if self.redeem_script:
                stream_serialize_field(PSBT_InKeyType.REDEEM_SCRIPT, f, value=self.redeem_script)

            if self.witness_script:
                stream_serialize_field(PSBT_InKeyType.WITNESS_SCRIPT, f, value=self.witness_script)

            for pub, dinfo in self.derivation_map.items():
                assert isinstance(pub, CPubKey) and pub.is_fullyvalid()
                stream_serialize_field(
                    PSBT_InKeyType.BIP32_DERIVATION, f, key_data=pub, value=dinfo.serialize()
                )

        if self.final_script_sig:
            stream_serialize_field(PSBT_InKeyType.FINAL_SCRIPTSIG, f, value=self.final_script_sig)

        if self.final_script_witness:
            stream_serialize_field(
                PSBT_InKeyType.FINAL_SCRIPTWITNESS, f, value=self.final_script_witness.serialize()
            )

        if self.proof_of_reserves_commitment:
            stream_serialize_field(
                PSBT_InKeyType.POR_COMMITMENT, f, value=self.proof_of_reserves_commitment
            )
    ...
    def get_amount(self, unsigned_tx: Optional[CTransaction]) -> int:
        ...
        if self.index is None:
            raise ValueError(
                "index field is not set on PSBT_Input,  cannot know which CTxIn to access"
            )
    ...
    def _repr_dict(self) -> "OrderedDict[str, str]":
        partial_sigs = ", ".join(
            f"x('{b2x(k)}'): x('{b2x(v)}')" for k, v in self.partial_sigs.items()
        )
        return OrderedDict(
            {
                "utxo": f"{self.utxo}",
                ...
                "derivation_map": f"{{{derivation_map_repr(self.derivation_map)}}}",
                "final_script_sig": f"x('{b2x(self.final_script_sig)}')",
                "final_script_witness": repr(self.final_script_witness),
                "proof_of_reserves_commitment": f"x('{b2x(self.proof_of_reserves_commitment)}')",
                "proprietary_fields": f"{{{proprietary_field_repr(self.proprietary_fields)}}}",
...
class PSBT_Output(PSBT_CoinClass, next_dispatch_final=True):
    ...
    def merge(
        self: T_PSBT_Output, other: T_PSBT_Output, allow_blob_duplicates: bool = False
    ) -> None:
    ...
    @classmethod
    def from_instance(cls: Type[T_PSBT_Output], inst: T_PSBT_Output) -> T_PSBT_Output:
        new_inst = cls()
        new_inst.merge(inst, allow_blob_duplicates=True)
        return new_inst
    ...
    def _check_sanity(self, unsigned_tx: CTransaction) -> None:
        ...
        if self.index is None:
            raise ValueError("index is not set for this instance of PSBT_Output")

        vout = unsigned_tx.vout[self.index]

        if not MoneyRange(vout.nValue):
            raise ValueError(f"Value of output at index {self.index} is out of valid range")
        ...
        if spk.is_witness_scriptpubkey():
            if rds:
                raise ValueError(
                    f"redeem script is specified for native segwit output at index {self.index}"
                )
            ...
            if rds:
                raise ValueError(
                    f"redeem script is specified for p2pkh output at index {self.index}"
                )
            if ws:
                raise ValueError(
                    f"witness script is specified for p2pkh output at index {self.index}"
                )
        elif spk.is_p2sh():
            if not rds:
                raise ValueError(
                    f"redeem script is not specified for p2sh output at index {self.index}"
                )
            ...
            if rds.is_witness_scriptpubkey():
                if rds.is_witness_v0_keyhash():
                    ...
                    raise ValueError("unsupported scriptPubKey type (that was p2sh-wrapped)")
            elif ws:
                raise ValueError(
                    f"witness script is specified for non-segwit p2sh output at index {self.index}"
                )
        else:
            raise ValueError("unsupported scriptPubKey type")
    ...
    def stream_deserialize(
        ...
        keys_seen: Set[bytes] = set()
        for key_type, key_data, value in read_psbt_keymap(
            f, keys_seen, PSBT_OutKeyType, proprietary_fields, unknown_fields
        ):
    ...
    def stream_serialize(self, f: ByteStream_Type, **kwargs: Any) -> None:
        if self.redeem_script:
            stream_serialize_field(PSBT_OutKeyType.REDEEM_SCRIPT, f, value=self.redeem_script)

        if self.witness_script:
            stream_serialize_field(PSBT_OutKeyType.WITNESS_SCRIPT, f, value=self.witness_script)

        for pub, dinfo in self.derivation_map.items():
            assert isinstance(pub, CPubKey) and pub.is_fullyvalid()
            stream_serialize_field(
                PSBT_OutKeyType.BIP32_DERIVATION, f, key_data=pub, value=dinfo.serialize()
            )
...
T_PartiallySignedTransaction = TypeVar(
    "T_PartiallySignedTransaction", bound="PartiallySignedTransaction"
)
...
class PartiallySignedTransaction(PSBT_CoinClass, next_dispatch_final=True):
    ...
    def __init__(
        ...
        *,
        ...
        unsigned_tx: Optional[CTransaction] = None,
        xpubs: Optional[Dict[CCoinExtPubKey, PSBT_KeyDerivationInfo]] = None,
        proprietary_fields: Optional[Dict[bytes, List[PSBT_ProprietaryTypeData]]] = None,
        unknown_fields: Optional[List[PSBT_UnknownTypeData]] = None,
        relaxed_sanity_checks: bool = False,
    ) -> None:
        ...
        if num_inputs != len(unsigned_tx.vin):
            raise ValueError(
                "length of inputs list supplied is not the same as number of inputs in unsigned_tx"
            )
        ...
        if unsigned_tx.has_witness():
            raise ValueError("Unsigned transaction contains witness data")
        if any(inp.scriptSig for inp in unsigned_tx.vin):
            raise ValueError("Unsigned transaction contains non-empty scriptSigs")
        ...
        for i in range(num_inputs):
            if inputs is None:
                ...
                ensure_isinstance(inputs[i], PSBT_Input, f"input at position {i}")
                inp = inputs[i].clone()
                if inp.index is None:
                    inp.index = i
                elif inp.index != i:
                    raise ValueError(f"incorrect index on PSBT_Input at position {i}")
        ...
        for i in range(num_outputs):
            if outputs is None:
                ...
                ensure_isinstance(outputs[i], PSBT_Output, f"output at position {i}")
                outp = outputs[i].clone()
                if outp.index is None:
                    outp.index = i
                elif outp.index != i:
                    raise ValueError(f"incorrect index on PSBT_Output at position {i}")
        ...
        for xpub, derinfo in xpubs.items():
            ...
            ensure_isinstance(derinfo, PSBT_KeyDerivationInfo, "derivation info for one of xpubs")
        self.xpubs = xpubs
        ...
        for prefix, p_fields in proprietary_fields.items():
            ...
            for field_index, field in enumerate(p_fields):
                ensure_isinstance(
                    field,
                    PSBT_ProprietaryTypeData,
                    f"one of proprietary field contents for prefix {b2x(prefix)}",
                )
        ...
        for u_field in unknown_fields:
            ensure_isinstance(u_field, PSBT_UnknownTypeData, "contents of unkown type")
        self.unknown_fields = unknown_fields
    ...
    @classmethod
    def from_instance(
        cls: Type[T_PartiallySignedTransaction], inst: T_PartiallySignedTransaction
    ) -> T_PartiallySignedTransaction:
        new_inst = cls()
        new_inst.merge(inst, allow_blob_duplicates=True)
        return new_inst

    def clone(self: T_PartiallySignedTransaction) -> T_PartiallySignedTransaction:
        return self.__class__.from_instance(self)

    def merge(
        self: T_PartiallySignedTransaction,
        ...
        for index, inp in enumerate(self.inputs):
            inp.merge(other.inputs[index], allow_blob_duplicates=allow_blob_duplicates)
        ...
        for index, outp in enumerate(self.outputs):
            outp.merge(other.outputs[index], allow_blob_duplicates=allow_blob_duplicates)

        for xpub, dinfo in other.xpubs.items():
            if xpub in self.xpubs:
                if self.xpubs[xpub].master_fp != dinfo.master_fp:
                    raise ValueError(
                        f"master fingerprint do not match in derivation info for xpub {str(xpub)}"
                    )
                if tuple(self.xpubs[xpub].path) != tuple(dinfo.path):
                    raise ValueError(
                        f"derivation paths do not match in derivation info for xpub {str(xpub)}"
                    )
            else:
    ...
    def combine(
        self: T_PartiallySignedTransaction, other: T_PartiallySignedTransaction
    ) -> T_PartiallySignedTransaction:
        new_psbt = self.clone()
    ...
    def from_base64_or_binary(
        cls: Type[T_PartiallySignedTransaction],
        ...
        **kwargs: Any,
        ...
        if isinstance(data, str):
            if (
                data[: len(CoreCoinParams.PSBT_MAGIC_HEADER_BASE64)]
        ...
        elif data_b.startswith(CoreCoinParams.PSBT_MAGIC_HEADER_BASE64.encode("ascii")):
            return cls.deserialize(
                base64.b64decode(data_b.decode("ascii"), validate=validate), **kwargs
            )
        else:
            raise ValueError("magic bytes at the start do not match PSBT magic bytes")

    @classmethod
    def from_binary(
        cls: Type[T_PartiallySignedTransaction], data: bytes, **kwargs: Any
    ) -> T_PartiallySignedTransaction:
        return cls.deserialize(data, **kwargs)

    @classmethod
    def from_base64(
        cls: Type[T_PartiallySignedTransaction], b64_data: str, validate: bool = True, **kwargs: Any
    ) -> T_PartiallySignedTransaction:
        return cls.deserialize(base64.b64decode(b64_data, validate=validate), **kwargs)
    ...
    def stream_deserialize(
        ...
        **kwargs: Any,
        ...
        keys_seen: Set[bytes] = set()
        for key_type, key_data, value in read_psbt_keymap(
            f, keys_seen, PSBT_GlobalKeyType, proprietary_fields, unknown_fields
        ):
            if key_type is PSBT_GlobalKeyType.UNSIGNED_TX:
                ...
                if key_data[:4] not in acceptable_prefixes:
                    if len(acceptable_xpub_prefixes) > 1:
                        ...
                        pfx_msg += ", ".join([b2x(pfx) for pfx in acceptable_prefixes])
                        pfx_msg += ")"
                    else:
                ...
                ensure_empty_key_data(key_type, key_data)
                if len(value) != 4:
                    raise SerializationError(f"Incorrect data length for {key_type.name}")
                version = struct.unpack(b"<I", value)[0]
            else:
                assert_never(key_type)

        if unsigned_tx is None:
            raise ValueError("PSBT does not contain unsigned transaction")
        ...
        for output_index in range(len(unsigned_tx.vout)):
            outputs.append(PSBT_Output.stream_deserialize(f, index=output_index, **kwargs))
    ...
    def stream_serialize(
        ...
        stream_serialize_field(
            PSBT_GlobalKeyType.UNSIGNED_TX,
            f,
        ...
        if self.version:
            stream_serialize_field(
                PSBT_GlobalKeyType.VERSION, f, value=struct.pack(b"<I", self.version)
            )

        for xpub, derinfo in self.xpubs.items():
            assert isinstance(xpub, CCoinExtPubKey)
            stream_serialize_field(
                PSBT_GlobalKeyType.XPUB,
                f,
        ...
        always_include_witness_utxo = kwargs.pop("always_include_witness_utxo", True)

        for inp in self.inputs:
            inp.stream_serialize(
                f, always_include_witness_utxo=always_include_witness_utxo, **kwargs
            )
    ...
    def sign(
        ...
        is_ready = len(self.unsigned_tx.vin) == num_inputs_final + num_inputs_ready
        return PSBT_SignResult(
            inputs_info=inputs_sign_info,
    ...
    def extract_transaction(self) -> CTransaction:
        ...
        if not sign_result.is_final:
            raise ValueError("not all inputs have required signatures")
...
class PartiallySignedBitcoinTransaction(PartiallySignedTransaction, PSBT_BitcoinClass): ...
