...
ScriptElement_Type = Union["CScriptOp", "bitcointx.core.key.CPubKey", int, bytes, bytearray]
...
class DATA(bytes):
    ...
    def __new__(cls, data: Union[bytes, bytearray]) -> "DATA":
        if not isinstance(data, (bytes, bytearray)):
            raise TypeError("DATA can only accept bytes or bytearray instance")
...
class NUMBER(int):
    ...
    def __new__(cls, num: int) -> "NUMBER":
        if not isinstance(num, int):
            raise TypeError(
                "NUMBER can only accept values that are instance of int class (except CScriptOp)"
            )
...
def OPCODE(op: CScriptOp) -> CScriptOp:
    ...
    if not isinstance(op, CScriptOp):
        raise TypeError("OPCODE can only accept instances of CScriptOp")
    return op
...
class CScript(bytes, ScriptCoinClass, next_dispatch_final=True):
    ...
    def __new__(
        ...
        *,
        ...
        if isinstance(value, (bytes, bytearray)):
            ...
            def coerce_iterable(
                iterable: Iterable[ScriptElement_Type],
            ...
            instance = super().__new__(cls, b"".join(coerce_iterable(value)))
    ...
    def raw_iter(self) -> Generator[Tuple[CScriptOp, Optional[bytes], int], None, None]:
        """Raw iteration
        ...
        while i < len(self):
            ...
            if opcode > OP_PUSHDATA4:
                ...
                if opcode < OP_PUSHDATA1:
                    ...
                    datasize = (
                        self[i] + (self[i + 1] << 8) + (self[i + 2] << 16) + (self[i + 3] << 24)
                    )
                    i += 4
    ...
    def __iter__(
        ...
        Union[CScriptOp, int, bytes],  # type: ignore
        None,
        None,
    ]:
        """'Cooked' iteration
    ...
    def is_p2sh(self) -> bool:
        ...
        """
        return (
            len(self) == 23 and self[0] == OP_HASH160 and self[1] == 0x14 and self[22] == OP_EQUAL
        )
    ...
    def to_p2sh_scriptPubKey(self: T_CScript, checksize: bool = True) -> T_CScript:
    ...
    def to_p2wsh_scriptPubKey(self: T_CScript, checksize: bool = True) -> T_CScript:
    ...
    def raw_sighash(
        ...
        return RawBitcoinSignatureHash(
            self, txTo, inIdx, hashtype, amount=amount, sigversion=sigversion
        )
...
class CScriptWitness(ImmutableSerializable):
    ...
    def __repr__(self) -> str:
        return (
            "CScriptWitness(["
            + ",".join("x('%s')" % bitcointx.core.b2x(s) for s in self.stack)
            + "])"
        )
    ...
    def stream_deserialize(
        cls: Type[T_CScriptWitness], f: ByteStream_Type, **kwargs: Any
    ) -> T_CScriptWitness:
        n = VarIntSerializer.stream_deserialize(f, **kwargs)
        stack = tuple(BytesSerializer.stream_deserialize(f, **kwargs) for i in range(n))
        return cls(stack)
...
def IsLowDERSignature(sig: bytes) -> bool:
    ...
    return CompareBigEndian(s_val, [0]) > 0 and CompareBigEndian(s_val, max_mod_half_order) <= 0
...
def RawBitcoinSignatureHash(
    ...
    if script.is_witness_scriptpubkey():
        raise ValueError("segwit scriptpubkey supplied with sigversion=SIGVERION_BASE")
    ...
    txtmp.vin[inIdx].scriptSig = FindAndDelete(script, script.__class__([OP_CODESEPARATOR]))
...
def SignatureHashSchnorr(
    ...
    *,
    ...
    if len(spent_outputs) != len(txTo.vin):
        raise ValueError("number of spent_outputs is not equal to number of inputs")
    ...
    if output_type == SIGHASH_ALL:
        f.write(hashlib.sha256(b"".join(txout.serialize() for txout in txTo.vout)).digest())
...
class StandardMultisigScriptInfo:
    ...
    def __init__(
        self, *, total: int, required: int, pubkeys: Sequence["bitcointx.core.key.CPubKey"]
    ):
...
def parse_standard_multisig_redeem_script(script: CScript) -> StandardMultisigScriptInfo:
    ...
    try:
        ...
        raise ValueError("script is too short: stoped at required number of signatures")
    if not isinstance(required, int):
        raise ValueError(
            "script is not a p2sh script, required number of signatures is not a number"
        )
    if required < 1:
    ...
    for i in range(MAX_P2SH_MULTISIG_PUBKEYS + 1):  # +1 for `total`
        try:
            ...
            if not pub.is_fullyvalid():
                raise ValueError(f"encountered an invalid pubkey at position {i}")
            pubkeys.append(pub)
        except StopIteration:
            raise ValueError(
    ...
    if total != len(pubkeys):
        raise ValueError(
            "number of pubkeys in the script is not the same as the number of pubkeys required"
        )
    if total < 2:
    ...
    try:
        ...
        raise ValueError("script is too short, ended before we could get the last opcode")

    if last_op != OP_CHECKMULTISIG:
        raise ValueError("script is not a p2sh script, last opcode is not OP_CHECKMULTISIG")
    try:
        next(si)
    ...
    return StandardMultisigScriptInfo(total=total, required=required, pubkeys=tuple(pubkeys))
...
def standard_multisig_redeem_script(
    *,
    ...
    if required > total:
        raise ValueError("'required' argument must be less than or equal to 'total'")
    if total > MAX_P2SH_MULTISIG_PUBKEYS:
        raise ValueError("%d pubkeys do not fit into standard p2sh multisig" % total)
    if total == 1:
...
class ComplexScriptSignatureHelper:
    ...
    def sign(
        self,
        signer: Callable[["bitcointx.core.key.CPubKey"], Optional[bytes]],
        partial_sigs: Optional[Dict["bitcointx.core.key.CPubKey", bytes]] = None,
...
class StandardMultisigSignatureHelper(ComplexScriptSignatureHelper):
    ...
    def get_pubkeys_without_sig(self) -> Iterable["bitcointx.core.key.CPubKey"]:
        return (p for i, p in enumerate(self._script_info.pubkeys) if not self._signatures[i])
...
class TaprootScriptTree(ScriptCoinClass, next_dispatch_final=True):
    ...
    def __init__(
        ...
        *,
        ...
        self._leaves_with_paths = tuple((leaf, merkle_paths[i]) for i, leaf in enumerate(leaves))
    ...
    def get_script_with_control_block(self, name: str) -> Optional[Tuple[CScript, bytes]]:
        ...
        if not self.internal_pubkey:
            raise ValueError(
                f"This instance of {self.__class__.__name__} does not have internal_pubkey"
            )
