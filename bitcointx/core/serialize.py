...
T_ImmutableSerializable = TypeVar("T_ImmutableSerializable", bound="ImmutableSerializable")
...
class DeserializationValueBoundsError(SerializationError):
    ...
    def __init__(
        ...
        msg: str,
        *,
        klass: Type["Serializer[T_unbounded]"],
        value: int,
        upper_bound: int,
        lower_bound: int,
    ):
...
class Serializable:
    ...
    def stream_deserialize(
        cls: Type[T_Serializable], f: ByteStream_Type, **kwargs: Any
    ) -> T_Serializable:
...
class Serializer(Generic[T_unbounded]):
    ...
    def __new__(cls: Type["Serializer[T_unbounded]"]) -> "Serializer[T_unbounded]":
        raise NotImplementedError

    @classmethod
    def stream_serialize(cls, obj: T_unbounded, f: ByteStream_Type, **kwargs: Any) -> None:
        raise NotImplementedError

    @classmethod
    def stream_deserialize(cls, f: ByteStream_Type, **kwargs: Any) -> T_unbounded:
        raise NotImplementedError
    ...
    @classmethod
    def deserialize_partial(cls, buf: bytes, **kwargs: Any) -> Tuple[T_unbounded, bytes]:
    ...
    def deserialize(cls, buf: bytes, **kwargs: Any) -> T_unbounded:
        ...
        if tail:
            raise ValueError(
                f"stray data after deserialization: {len(buf)} byte(s) unaccounted for"
            )
        return inst
...
class VarIntSerializer(Serializer[int]):
    ...
    @classmethod
    def stream_serialize(cls, obj: int, f: ByteStream_Type, **kwargs: Any) -> None:
        i = obj
        if i < 0:
    ...
    @classmethod
    def stream_deserialize(
        cls, f: ByteStream_Type, allow_full_range: bool = False, **kwargs: Any
    ) -> int:
        r = ser_read(f, 1)[0]
...
class BytesSerializer(Serializer[bytes]):
    ...
    @classmethod
    def stream_serialize(cls, obj: bytes, f: ByteStream_Type, **kwargs: Any) -> None:
...
class VectorSerializer(Serializer[Sequence[Serializable]]):
    ...
    def stream_serialize(
        cls, obj: Sequence[Serializable], f: ByteStream_Type, **kwargs: Any
    ) -> None:
        obj_seq = obj
        ...
        for inst in obj_seq:
            ...
            if cur_cls is not inner_cls:
                ...
                if imm_cls and imm_cls is getattr(inner_cls, "_immutable_cls", None):
    ...
    @classmethod
    def stream_deserialize(
        cls, f: ByteStream_Type, element_class: Optional[Type[T_Serializable]] = None, **kwargs: Any
    ) -> Sequence[T_Serializable]:
        if element_class is None:
            raise ValueError("The class of the elements in the vector must be supplied")
...
class uint256VectorSerializer(Serializer[Sequence[bytes]]):
    ...
    @classmethod
    def stream_serialize(cls, obj: Sequence[bytes], f: ByteStream_Type, **kwargs: Any) -> None:
        uints = obj
    ...
    @classmethod
    def stream_deserialize(cls, f: ByteStream_Type, **kwargs: Any) -> List[bytes]:
        n = VarIntSerializer.stream_deserialize(f, **kwargs)
        r = []
        for i in range(n):
...
class intVectorSerializer(Serializer[Sequence[int]]):
    @classmethod
    def stream_serialize(cls, obj: Sequence[int], f: ByteStream_Type, **kwargs: Any) -> None:
        ints = obj
        datalen = len(ints)
...
def make_mutable(cls: Type[T_ImmutableSerializable]) -> Type[T_ImmutableSerializable]:
    if not issubclass(cls, ImmutableSerializable):
        raise TypeError("make_mutable can only be applied to subclasses of ImmutableSerializable")
