...
class HTTPClient_Protocol(Protocol):
    def request(self, method: str, path: str, postdata: str, headers: Dict[str, str]) -> None: ...
...
class JSONRPCError(Exception):
    ...
    @classmethod
    def _register_subcls(cls, subcls: Type["JSONRPCError"]) -> Type["JSONRPCError"]:
...
def _try_read_conf_file(
    conf_file: Optional[str], conf_file_contents: Optional[str], allow_default_conf: bool
) -> Dict[str, str]:
...
class RPCCaller:
    ...
    def __init__(
        ...
        if service_url is None:
            ...
            if conf_file is None and conf_file_contents is None:
                if not allow_default_conf:
                    raise ValueError(
                        "if conf_file is not specified, allow_default_conf must be True"
                    )
                conf_file = params.get_config_path()

            conf = _try_read_conf_file(conf_file, conf_file_contents, allow_default_conf)
            ...
            (host, port) = split_hostport(
                conf.get(f"{network_id}.rpcconnect", conf.get("rpcconnect", "localhost"))
            )

            port = int(conf.get(f"{network_id}.rpcport", conf.get("rpcport", port or service_port)))
            service_url = "%s://%s:%d" % ("http", host, port)

            cookie_dir = conf.get(
                f"{network_id}.datadir",
                conf.get("datadir", None if conf_file is None else os.path.dirname(conf_file)),
            )
            io_err = None
            ...
            if authpair is None:
                if f"{network_id}.rpcpassword" in conf:
                    ...
                    authpair = "%s:%s" % (conf.get("rpcuser", ""), conf["rpcpassword"])
                elif io_err is None:
                    raise ValueError(
                    ...
                    raise ValueError(
                        ...
                        "not specified in the configuration file: %r" % (io_err, conf_file)
                    )
        else:
        ...
        if authpair is None:
            ...
            self.__auth_header = "Basic " + base64.b64encode(authpair.encode("utf8")).decode("utf8")
    ...
    def connect(self, connection: Optional[HTTPClient_Type] = None) -> None:
        if connection:
            ...
            self.__conn = http.client.HTTPConnection(
                self.__url.hostname or "", port=self.__port, timeout=self.__timeout
            )
    ...
    def _call(self, service_name: str, *args: Any) -> Any:
        ...
        postdata = json.dumps(
            {"version": "1.1", "method": service_name, "params": args, "id": self.__id_count},
    ...
    def _get_response(self) -> Any:
        ...
        if http_response is None:
            raise JSONRPCError({"code": -342, "message": "missing HTTP response from server"})
            ...
            raise JSONRPCError(
                {
                    "code": -342,
                    "message": (
                        "non-JSON HTTP response with '%i %s' "
                        "from server: '%.20s%s'"
                        % (
