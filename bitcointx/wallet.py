...
class WalletCoinClassDispatcher(
    ClassMappingDispatcher, identity="wallet", depends=[bitcointx.core.CoreCoinClassDispatcher]
): ...


class WalletBitcoinClassDispatcher(
    WalletCoinClassDispatcher, depends=[bitcointx.core.CoreBitcoinClassDispatcher]
): ...
...
class WalletBitcoinClass(WalletCoinClass, metaclass=WalletBitcoinClassDispatcher): ...


class WalletBitcoinTestnetClass(
    WalletBitcoinClass, metaclass=WalletBitcoinTestnetClassDispatcher
): ...


class WalletBitcoinRegtestClass(
    WalletBitcoinClass, metaclass=WalletBitcoinRegtestClassDispatcher
): ...


class WalletBitcoinSignetClass(
    WalletBitcoinClass, metaclass=WalletBitcoinSignetClassDispatcher
): ...
...
class CCoinAddress(WalletCoinClass):
    ...
    def __new__(cls: Type[T_CCoinAddress], s: str) -> T_CCoinAddress:
        ...
        for target_cls in target_cls_set:
            try:
                ...
                assert isinstance(inst, bytes), "{inst.__class__.__name__} must be bytes subclass"
    ...
    @classmethod
    def from_scriptPubKey(cls: Type[T_CCoinAddress], scriptPubKey: CScript) -> T_CCoinAddress:
        ...
        raise CCoinAddressError("scriptPubKey is not in a recognized address format")
    ...
    @classmethod
    def match_scriptPubKey_type(cls, spk_type_string: str) -> Optional[Type["CCoinAddress"]]:
...
T_CBase58DataDispatched = TypeVar("T_CBase58DataDispatched", bound="CBase58DataDispatched")


class CBase58DataDispatched(bitcointx.base58.CBase58Data):
    def __init__(self, _s: str) -> None:
        if not self.base58_prefix:
            raise TypeError(f"{self.__class__.__name__} must not be instantiated directly")
        if len(self) != self.__class__._data_length:
            raise TypeError(f"lengh of the data is not {self.__class__._data_length}")
    ...
    def base58_get_match_candidates(
        ...
        if not candidates:
            if not cls.base58_prefix:
                raise TypeError(
                    "if class has no dispatched descendants, it must have base58_prefix set"
                )
            candidates = [cls]
        return candidates


T_CBech32DataDispatched = TypeVar("T_CBech32DataDispatched", bound="CBech32DataDispatched")
...
class CBech32DataDispatched(bitcointx.bech32.CBech32Data):
    ...
    @classmethod
    def bech32_get_match_candidates(
        cls: Type[T_CBech32DataDispatched],
...
class P2SHCoinAddress(CBase58CoinAddress, next_dispatch_final=True):
    ...
    @classmethod
    def from_redeemScript(cls: Type[T_P2SHCoinAddress], redeemScript: CScript) -> T_P2SHCoinAddress:
    ...
    @classmethod
    def from_scriptPubKey(cls: Type[T_P2SHCoinAddress], scriptPubKey: CScript) -> T_P2SHCoinAddress:
...
class P2PKHCoinAddress(CBase58CoinAddress, next_dispatch_final=True):
    ...
    def from_scriptPubKey(
        cls: Type[T_P2PKHCoinAddress], scriptPubKey: CScript
    ) -> T_P2PKHCoinAddress:
    ...
    @classmethod
    def from_redeemScript(
        cls: Type[T_P2PKHCoinAddress], redeemScript: CScript
    ) -> T_P2PKHCoinAddress:
...
class P2WSHCoinAddress(CBech32CoinAddress, next_dispatch_final=True):
    ...
    @classmethod
    def from_scriptPubKey(
        cls: Type[T_P2WSHCoinAddress], scriptPubKey: CScript
    ) -> T_P2WSHCoinAddress:
    ...
    @classmethod
    def from_redeemScript(
        cls: Type[T_P2WSHCoinAddress], redeemScript: CScript
    ) -> T_P2WSHCoinAddress:
...
class P2WPKHCoinAddress(CBech32CoinAddress, next_dispatch_final=True):
        ...
        *,
        ...
        if not accept_invalid:
            ...
            if not pubkey.is_compressed():
                raise P2PKHCoinAddressError("Uncompressed pubkeys are not allowed")
    ...
    @classmethod
    def from_scriptPubKey(
        cls: Type[T_P2WPKHCoinAddress], scriptPubKey: CScript
    ) -> T_P2WPKHCoinAddress:
    ...
    @classmethod
    def from_redeemScript(
        cls: Type[T_P2WPKHCoinAddress], redeemScript: CScript
    ) -> T_P2WPKHCoinAddress:
...
class P2TRCoinAddress(CBech32CoinAddress, next_dispatch_final=True):
    ...
    def from_xonly_pubkey(
        cls: Type[T_P2TRCoinAddress], pubkey: Union[XOnlyPubKey, bytes, bytearray]
    ) -> T_P2TRCoinAddress:
    ...
    def from_output_pubkey(
        ...
        *,
        ...
        is True.
        """
        ensure_isinstance(pubkey, (XOnlyPubKey, CPubKey, bytes, bytearray), "pubkey")

        if len(pubkey) == 32:
            if not isinstance(pubkey, CPubKey):  # might be invalid CPubKey
                return cls.from_xonly_output_pubkey(pubkey, accept_invalid=accept_invalid)
        ...
        if not accept_invalid:
            ...
            if not pubkey.is_fullyvalid():
                raise P2TRCoinAddressError("invalid pubkey")
            if not pubkey.is_compressed():
                raise P2TRCoinAddressError("Uncompressed pubkeys are not allowed")
        ...
        return cls.from_xonly_output_pubkey(XOnlyPubKey(pubkey), accept_invalid=accept_invalid)
    ...
    @classmethod
    def from_script_tree(
        cls: Type[T_P2TRCoinAddress], stree: TaprootScriptTree
    ) -> T_P2TRCoinAddress:
        """Create a P2TR address from TaprootScriptTree instance"""
        if not stree.internal_pubkey:
            raise P2TRCoinAddressError(
                f"The supplied instance of {stree.__class__.__name__} does not have internal_pubkey"
            )
        assert stree.output_pubkey is not None
        return cls.from_xonly_output_pubkey(stree.output_pubkey)

    @classmethod
    def from_scriptPubKey(cls: Type[T_P2TRCoinAddress], scriptPubKey: CScript) -> T_P2TRCoinAddress:
    ...
    def to_redeemScript(self) -> None:  # type: ignore
        raise NotImplementedError("not enough data in p2tr address to reconstruct redeem script")
...
class CBech32BitcoinTestnetAddress(CBech32CoinAddress, CBitcoinTestnetAddress):
    bech32_hrp = "tb"


class CBech32BitcoinSignetAddress(CBech32CoinAddress, CBitcoinSignetAddress):
    bech32_hrp = "tb"


class CBech32BitcoinRegtestAddress(CBech32CoinAddress, CBitcoinRegtestAddress):
    bech32_hrp = "bcrt"
...
class P2PKHBitcoinTestnetAddress(P2PKHCoinAddress, CBase58BitcoinTestnetAddress):
    base58_prefix = bytes([111])


class P2SHBitcoinTestnetAddress(P2SHCoinAddress, CBase58BitcoinTestnetAddress):
    base58_prefix = bytes([196])


class P2PKHBitcoinRegtestAddress(P2PKHCoinAddress, CBase58BitcoinRegtestAddress):
    base58_prefix = bytes([111])


class P2PKHBitcoinSignetAddress(P2PKHCoinAddress, CBase58BitcoinSignetAddress):
    base58_prefix = bytes([111])


class P2SHBitcoinRegtestAddress(P2SHCoinAddress, CBase58BitcoinRegtestAddress):
    base58_prefix = bytes([196])


class P2SHBitcoinSignetAddress(P2SHCoinAddress, CBase58BitcoinSignetAddress):
    base58_prefix = bytes([196])
...
class P2WSHBitcoinTestnetAddress(P2WSHCoinAddress, CBech32BitcoinTestnetAddress): ...


class P2WPKHBitcoinTestnetAddress(P2WPKHCoinAddress, CBech32BitcoinTestnetAddress): ...


class P2TRBitcoinTestnetAddress(P2TRCoinAddress, CBech32BitcoinTestnetAddress): ...


class P2WSHBitcoinRegtestAddress(P2WSHCoinAddress, CBech32BitcoinRegtestAddress): ...


class P2WPKHBitcoinRegtestAddress(P2WPKHCoinAddress, CBech32BitcoinRegtestAddress): ...


class P2TRBitcoinRegtestAddress(P2TRCoinAddress, CBech32BitcoinRegtestAddress): ...


class P2WSHBitcoinSignetAddress(P2WSHCoinAddress, CBech32BitcoinSignetAddress): ...


class P2WPKHBitcoinSignetAddress(P2WPKHCoinAddress, CBech32BitcoinSignetAddress): ...


class P2TRBitcoinSignetAddress(P2TRCoinAddress, CBech32BitcoinSignetAddress): ...


T_CCoinKey = TypeVar("T_CCoinKey", bound="CCoinKey")


class CCoinKey(CBase58DataDispatched, CKeyBase, WalletCoinClass, next_dispatch_final=True):
    ...
    def from_secret_bytes(
        cls: Type[T_CCoinKey], secret: bytes, compressed: bool = True
    ) -> T_CCoinKey:
    ...
    def sign_schnorr_tweaked(
        ...
        *,
        ...
        """
        return self._sign_schnorr_internal(hash, merkle_root=merkle_root, aux=aux)
...
class CCoinExtPubKey(
    CBase58DataDispatched, CExtPubKeyBase, WalletCoinClass, next_dispatch_final=True
):
    def __init__(self, _s: str) -> None:
...
class CCoinExtKey(CBase58DataDispatched, CExtKeyBase, WalletCoinClass, next_dispatch_final=True):
    ...
    def _xpub_class(self) -> Type[CCoinExtPubKey]:
        return cast(Type[CCoinExtPubKey], dispatcher_mapped_list(CCoinExtPubKey)[0])

    @property
    def _key_class(self) -> Type[CCoinKey]:
        return cast(Type[CCoinKey], dispatcher_mapped_list(CCoinKey)[0])
