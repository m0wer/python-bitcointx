...
T_ClassMappingDispatcher = TypeVar("T_ClassMappingDispatcher", bound="ClassMappingDispatcher")
...
class no_bool_use_as_property:
    ...
    def __get__(self, instance: object, owner: type) -> _NoBoolCallable:
        ...
        name = "{}{}.{}".format(owner.__name__, "" if instance is None else "()", method.__name__)
        return _NoBoolCallable(name, cast(Callable[[], bool], method))


def get_class_dispatcher_depends(
...
def activate_class_dispatcher(
    dclass: Type[T_ClassMappingDispatcher],
    ...
    if dclass is not prev:
        ...
        class_mapping_dispatch_data.set_dispatcher_class(dclass._class_dispatcher__identity, dclass)

    return prev  # type: ignore


def dispatcher_mapped_list(
    cls: T_ClassMappingDispatcher,
...
class DispatcherMethodWrapper:
    ...
    def __init__(
        ...
        wrapper: Callable[[Callable[..., Any], type], Callable[..., Any]],
    ) -> None:
        self.method = method
...
def dispatcher_wrap_methods(
    cls: "ClassMappingDispatcher",
    wrap_fn: Callable[[Callable[..., Any], type], Callable[..., Any]],
    *,
    ...
    for attr_name, attr_value in classdict.items():
        if isinstance(
            attr_value, (FunctionType, classmethod, staticmethod, DispatcherMethodWrapper)
        ):
            setattr(cls, attr_name, DispatcherMethodWrapper(attr_value, wrap_fn))
...
class ClassMappingDispatcher(ABCMeta):
    ...
    _class_dispatcher__clsmap: Dict["ClassMappingDispatcher", List["ClassMappingDispatcher"]]
    _class_dispatcher__identity: str
    _class_dispatcher__depends: Iterable[Type["ClassMappingDispatcher"]]

    def __init_subclass__(
        mcs: Type["ClassMappingDispatcher"],
        ...
        if not getattr(mcs, "_class_dispatcher__identity", None):
            raise TypeError("identity attribute is not set for the base dispatcher class")
    ...
    def __init__(
        ...
        def wrap(fn: Callable[..., Any], mcs: Type["ClassMappingDispatcher"]) -> Callable[..., Any]:
            @functools.wraps(fn)
    ...
    def __getattribute__(cls, name: str) -> Any:
        ...
        if name.startswith("__") and name.endswith("__") or name in _attributes_of_ABC:
...
def ensure_isinstance(
    ...
    if not isinstance(var, type_or_types):
        if isinstance(type_or_types, type):  # single type
            msg = (
                f"{var_description} is expected to be an instance of "
...
class ReadOnlyField(Generic[T_unbounded]):
    ...
    def __get__(self: T_unbounded, instance: object, owner: type) -> T_unbounded:
        raise NotImplementedError

    def __set__(self: T_unbounded, instance: object, value: ReadOnlyFieldGuard) -> None:
        raise NotImplementedError
...
class ContextLocalClassDispatchers(ContextVarsCompat):
    ...
    def get_dispatcher_class(self, identity: str) -> Optional[Type[ClassMappingDispatcher]]:
    ...
    def set_dispatcher_class(self, identity: str, value: Type[ClassMappingDispatcher]) -> None:
        assert self.is_valid_identity(identity)
